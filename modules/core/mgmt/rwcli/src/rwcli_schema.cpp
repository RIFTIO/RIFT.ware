...
bool SchemaManager::load_all_schemas()
{
  ...
  // directory.
  fs::path const schema_path = fs::path(rift_install_) / fs::path(RW_SCHEMA_VER_LATEST_NB_PATH);
  fs::path const yang_dir = schema_path / YANG_DIR;

  try{
    // check if the latest directory has changed
    fs::path const version_directory = fs::path(rift_install_) / fs::path(RW_SCHEMA_VER_PATH);
