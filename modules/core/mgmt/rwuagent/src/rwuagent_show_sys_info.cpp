...
#define ASSERT_IOERR(cond_, ...) \
  ...
  if (!boost::filesystem::exists(zsh_path)) {
    free(zsh_path);
    ioerr = asprintf( &zsh_path, "%s", RW_MGMT_AGENT_ZSH_PATH );
    ASSERT_IOERR( ioerr > 0, "bad zsh path" );

    if (!boost::filesystem::exists(zsh_path)) {
      std::cout << "Unable to find RW.CLI executable" << std::endl;
      exit(-1);
    }
  }
  ...
  if (ioerr != 0) {
    std::cout << "Failed to execute RW.CLI -" << strerror(errno) <<  std::endl;
    exit(-1);
  }
  // Will not reach here
}
