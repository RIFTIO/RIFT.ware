    ...
    log_file_writers_()
{
  ...
  const auto log_path = instance_->mgmt_handler()->mgmt_dir() + "/var/agent/log";
  transaction_log_file_ = log_path + "/rwuagent_transaction.log";
  failure_log_file_ = log_path + "/rwuagent_transaction_failures.log";
...
void LogFileManager::add_confd_log_files() {
  ...
  std::string const log_dir = instance_->mgmt_handler()->mgmt_dir() + "/var/confd/log/";
  ...
  if (fs::exists(log_dir)) has_confd_logs_ = true;
}
    ...
    
{
  ...
  if (has_confd_logs_) {
    ...
    auto nc_trace_file = get_rift_var_root() + "/" + instance_->mgmt_handler()->mgmt_dir() +
                         "/var/confd/log/netconf.trace";
                                           ...
                                           const std::string& file_name)
{
  ...
  std::string ofile = get_rift_var_root() + "/" + file_name;
