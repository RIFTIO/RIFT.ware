...
static const int INTERNAL_HB_INTERVAL_SEC = 120;
...
{
  // Do not load any registration(s) if Agent
  // is started as standby
  ...
  while (nullptr != first_level) {
    ...
    if (first_level->is_config()) {
      ...
      flag = RWDTS_FLAG_PUBLISHER | RWDTS_FLAG_SHARED;
  ...
  /// Register for hearbeat events via DTS
  register_internal_heartbeat();
  // Register for agent config state
  register_agent_config_state();
}

void DtsMember::remove_registrations()
{
  std::for_each(std::begin(all_dts_regs_), std::end(all_dts_regs_), rwdts_member_deregister);
  all_dts_regs_.clear();
}


void DtsMember::do_manifest_registration()
{
  RW_ASSERT (!instance_->is_ha_master());
  RWMEMLOG (memlog_buf_, RWMEMLOG_MEM2, "do manifest registration");

  rw_keyspec_path_t *ks = nullptr;
  RWPB_T_PATHSPEC(RwManifest_Manifest) manifest;
  manifest = (*RWPB_G_PATHSPEC_VALUE(RwManifest_Manifest));
  ks = (rw_keyspec_path_t *)&manifest;
  ...
  reg = rwdts_member_register(
                    ...
                    ks,
                    &reg_cb,
                    RWPB_G_MSG_PBCMD(RwManifest_Manifest),
                    RWDTS_FLAG_PUBLISHER | RWDTS_FLAG_SHARED,
                    nullptr);
  RW_ASSERT (reg);
}

void DtsMember::register_agent_config_state()
{
  RWMEMLOG (memlog_buf_, RWMEMLOG_MEM2, "register agent config state");
  ...
  rwdts_member_event_cb_t reg_cb;
  RW_ZERO_VARIABLE (&reg_cb);
  reg_cb.ud = static_cast<void*>(this);
  ...
  all_dts_regs_.push_back(new_reg);
}
...
/// ATTN: Test code for debugging system test timeout issues
// See RIFT-14175, RIFT-14152 for more details
void DtsMember::register_internal_heartbeat()
{
  RWMEMLOG (memlog_buf_, RWMEMLOG_MEM2, "register internal heartbeat");
  RWPB_M_PATHSPEC_DECL_INIT (
      RwMgmtagt_data_Uagent_State_InternalHeartbeat, ks);
  ...
  auto new_reg = rwdts_member_register(
                        ...
                        keyspec,
                        &reg_cb,
                        RWPB_G_MSG_PBCMD (RwMgmtagt_data_Uagent_State_InternalHeartbeat),
                        RWDTS_FLAG_PUBLISHER,
                        nullptr);
  ...
  // Register as Subscriber as well which will perform some action
  // on receiving published transaction
  rwdts_member_event_cb_t sreg_cb;
  RW_ZERO_VARIABLE (&sreg_cb);
  sreg_cb.ud = static_cast<void*>(this);
  sreg_cb.cb.prepare = &DtsMember::internal_hb_subscription_cb;

  auto new_sreg = rwdts_member_register(
                        nullptr,
                        api_,
                        keyspec,
                        &sreg_cb,
  ...
  all_dts_regs_.push_back(new_sreg);
}

void DtsMember::retry_publish_internal_hb_event_cb(void* ctx)
{
  RW_ASSERT (ctx);
  auto self = static_cast<DtsMember*>(ctx);
  self->publish_internal_hb_event();
}

void DtsMember::publish_internal_hb_event()
{
  RWMEMLOG(memlog_buf_, RWMEMLOG_MEM2, "publish internal hb event");

  auto keyspec_entry = (*RWPB_G_PATHSPEC_VALUE(RwMgmtagt_data_Uagent_State_InternalHeartbeat));

  auto* keyspec = reinterpret_cast<rw_keyspec_path_t*>(&keyspec_entry);
  RW_ASSERT (keyspec);

  RWPB_T_MSG(RwMgmtagt_data_Uagent_State_InternalHeartbeat) hb_event;
  RWPB_F_MSG_INIT(RwMgmtagt_data_Uagent_State_InternalHeartbeat, &hb_event);
...
rwdts_member_rsp_code_t DtsMember::internal_hb_subscription_cb(
    ...
    void * get_next_key)
{
  ...
  self->instance_->log_file_manager()->log_string(self, "internal-hb-event", "");
  return RWDTS_ACTION_OK;
}

void DtsMember::internal_hb_publish_cb(rwdts_xact_t* xact,
                                       rwdts_xact_status_t* xact_status,
                                       void* ud)
{
  auto self = static_cast<DtsMember*>(ud);
  RW_ASSERT (self);
  RWMEMLOG(self->memlog_buf_, RWMEMLOG_MEM2, "internal hb publish cb");
  ...
  case RWDTS_XACT_ABORTED:
  {
    ...
    {
      RW_MA_INST_LOG (self->instance_, InstanceError, "Internal heartbeat publish failed.");
      if (err_str) {
  ...
  default:
    RW_MA_INST_LOG (self->instance_, InstanceDebug, "Internal heartbeat published successfully.");
    break;
  };

  auto when = dispatch_time(DISPATCH_TIME_NOW, NSEC_PER_SEC * INTERNAL_HB_INTERVAL_SEC);
  rwsched_dispatch_after_f(self->instance_->rwsched_tasklet(),
                           when,
                           rwsched_dispatch_get_main_queue(self->instance_->rwsched()),
                           self,
                           retry_publish_internal_hb_event_cb);
}
