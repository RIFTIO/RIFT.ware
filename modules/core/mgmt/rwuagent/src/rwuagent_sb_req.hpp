...
: public SbReq
{
 public:
  ...
  SbReqEditConfig (
      ...
      std::function<void()> prepare_cb = nullptr); /**< [in] 
                                     Callback provided by the client which will
                                     get executed on DTS prepare phase completion*/
  ...
  /*!
   * Increments the xact_block_count_ by 1
   * Should be executed from Main dispatch queue.
   */
  void incr_block_count() noexcept;

  /*!
   * Decrements the xact_block_count_ by 1
   * Should be exceuted from the Main dispatch queue.
   */
  void decr_block_count() noexcept;

  int xact_block_count() const noexcept { return xact_block_count_; }

  static void dts_block_prepare_cb(rwdts_xact_t* xact,
  ...
  bool add_block_to_xact(rw_keyspec_path_t* ks,
                         RWDtsQueryAction query_action,
                         ProtobufCMessage* message);
 
  bool add_block_to_xact(std::list<UniquePtrKeySpecPath::uptr_t>& ks_list,
                         RWDtsQueryAction query_action);
 ...
 private:
  ...
  /*!
   * Collects the transaction contents, validates the DOM
   * and then saves/commits it.
   */
  StartStatus process_dom();
  ...
  /*!
   */
  void get_errors_from_xact(rwdts_xact_t* xact, NetconfErrorList& nc_errors);
 ...
 private:
  ...
  /// Count of the transaction blocks created in the
  //running edit config SbReq
  int xact_block_count_ = 1;

  // Stores the callback which needs to be called
  // when all the transaction blocks gets executed
  // successfully.
  std::function<void()> on_prepare_cb_ = nullptr;

  // Indicates whether prepare phase is completed or not
