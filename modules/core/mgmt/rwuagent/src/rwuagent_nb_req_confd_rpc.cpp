                               ...
                               confd_tag_value_t *params, int nparams)
{
  ...
  RW_MA_NBREQ_LOG (this, ClientDebug, "RPC Operation from confd", 
      (capture_temporary=rpc->to_string()).c_str());

  SbReqRpc *xact = new SbReqRpc(instance_, this, RequestMode::CONFD, cmd.get());

  RWMEMLOG (memlog_buf_, RWMEMLOG_MEM2, "New sb rpc trx",
      RWMEMLOG_ARG_PRINTF_INTPTR("sbreq=0x%" PRIX64, (intptr_t)xact));
  ...
  rw_yang::XMLDocument::uptr_t rsp_dom )
{
  ...
  if (nullptr != reply) {
    std::string capture_temporary;
    RW_MA_NBREQ_LOG (this, ClientDebug, "Rpc response", 
        (capture_temporary=reply->to_string()).c_str());
  ...
  NetconfErrorList* nc_errors )
{
  ...
  std::string log = "RPC transaction failed: " + err_str;
