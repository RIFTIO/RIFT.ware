...
rw_status_t ConfdWorkspaceMgr::generate_config_file()
{
      ...
      && vcs_inst->pb_rwmanifest->init_phase->settings->mgmt_agent->has_netconf_trace)
  {
    ...
    address = vcs_inst->identity.vm_ip_address;
  }
...
bool ConfdWorkspaceMgr::set_schema_load_path()
{
  ...
  auto latest_ver_dir = get_rift_var_root() + "/" + RW_SCHEMA_VER_LATEST_NB_PATH + "/fxs";
...
void ConfdWorkspaceMgr::set_ip_addr(
    const std::string& node_path,
    const std::string& elem_name,
    const std::string& elem_value)
{
  RWMEMLOG(memlog_buf_, RWMEMLOG_MEM2, "set ip address");
...
bool ConfdWorkspaceMgr::create_confd_workspace()
{
  ...
  std::array<const char*, 6> sub_dirs = {
    "/var/agent/log",
    "/var/confd/candidate",
    "/var/confd/cdb",
    "/var/confd/log",
...
void ConfdWorkspaceMgr::enable_confd_ha(bool enable)
{
  XMLNode* root_node = confd_conf_dom_->get_root_node();
  RW_ASSERT (root_node);

  XMLNode* node = root_node->find("ha");
  RW_ASSERT (node);
