...
void
rwdts_kv_redis_get_api(rwdts_kv_handle_t *handle, int db_num, char *key, int key_len,
                       void *callbkfn, void *callbk_data)
{
  rwdts_kv_db_trans_t *trans_block = RW_MALLOC0(sizeof(rwdts_kv_db_trans_t));
  ...
  rwdts_redis_get_key_value(handle, db_num, key,
                                 ...
                                 void *callbk_data)
{
  ...
  rwdts_redis_get_hash_key_value(handle, db_num,
                                 key, key_len,
                                        ...
                                        void *callbk_data)
{
  ...
  rwdts_redis_get_hash_key_sernum_value(handle, db_num,
                                        key, key_len,
                                    ...
                                    void *callbk_data)
{
  ...
  rwdts_redis_hash_exist_key_value(handle, db_num,
...
rwdts_reply_status rwdts_kv_redis_load_sing_script_callbk_fn(rwdts_redis_msg_handle *redis_handle,
                                                            void *userdata)
{
  ...
  if (!trans_block->num_nodes) {
    if (NULL != RWDTS_VIEW_REPLY(redis_handle)) {
      ...
      if (len) {
        memcpy((char *)&trans_block->handle->single_command[0],
               redis_handle->reply->str,
               len);
        trans_block->handle->single_command[len] = '\0';
      }
    }
    RW_FREE(trans_block);
  }
...
void rwdts_kv_redis_init_cback_fn(rwdts_redis_db_init_info *db_info)
{
  ...
  if (RWDTS_REDIS_DB_DOWN == RWDTS_REDIS_DB_STATUS(db_info)) {
   ...
   char *set_shash_path, *del_shash_path, *get_shash_all_path, *sing_comm_path;

   rw_install_dir = getenv("RIFT_INSTALL");
   sing_comm_path = RW_STRDUP_PRINTF("%s/usr/bin/lua-scripts/%s",
                                     rw_install_dir, "single_command.lua");
   ...
   rwdts_kv_db_trans_t *trans_block = (rwdts_kv_db_trans_t *)init_info->userdata;
   rwdts_kv_redis_load_script(trans_block->handle, sing_comm_path,
                              rwdts_kv_redis_load_sing_script_callbk_fn);
                             ...
                             void *callbkfn, void *callbk_data)
{
  ...
  while(status == RW_STATUS_FAILURE) {
    status = rwdts_redis_run_shash_set_script((rwdts_redis_instance_t *)handle->kv_conn_instance,
                             ...
                             void *callbkfn, void *callbk_data)
{
  ...
  while(!trans_block->num_nodes) {
    rw_status_t status = rwdts_redis_run_del_shash_script((rwdts_redis_instance_t *)handle->kv_conn_instance,
    ...
    retry++;
    if ((status != RW_STATUS_SUCCESS) && (retry == MAX_DB_RETRIES)) {
      if (trans_block->u.del_callbkfn) {
        trans_block->u.del_callbkfn(RWDTS_KV_LIGHT_DEL_FAILURE, callbk_data);
      }
      RW_FREE(trans_block);
...
void rwdts_redis_wrapper_master_to_slave(void *handle, char *hostname, 
                                         ...
                                         void *userdata)
{
  rwdts_redis_master_to_slave((rwdts_redis_instance_t *)((rwdts_kv_handle_t *)handle)->kv_conn_instance,
                              hostname, port, (rwdts_trans_callback)callback,
                              userdata);
  return;
}
...
void rwdts_redis_wrapper_slave_to_master(void *handle, void *callback,
                                         void *userdata)
{
  rwdts_redis_slave_to_master((rwdts_redis_instance_t *)((rwdts_kv_handle_t *)handle)->kv_conn_instance,
                              (rwdts_trans_callback)callback, userdata);
  return;
} 
