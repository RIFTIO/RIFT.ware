...
static void rwdts_xact_deinit_cursor(rwdts_xact_t *xact);
...
*rwdts_xact_ref(rwdts_xact_t *x, const char *file, int line)
{
  ...
  int indx = g_atomic_int_get(&x->ref_indx) ;
  if (indx < RWDTS_XACT_REF_DEBUG_CNT) {
    g_atomic_int_inc(&x->ref_indx);
    g_atomic_pointer_set(&x->ref_debug[indx].fn, file);
    g_atomic_int_set(&x->ref_debug[indx].line, line);
    g_atomic_int_set(&x->ref_debug[indx].ref, 1);
  }
...
rwdts_xact_unref(rwdts_xact_t *x, const char *file, int line)
{
  ...
  if (indx < RWDTS_XACT_REF_DEBUG_CNT) {
    ...
    g_atomic_pointer_set(&x->ref_debug[indx].fn, file);
    g_atomic_int_set(&x->ref_debug[indx].line, line);
    g_atomic_int_set(&x->ref_debug[indx].ref, 2);
  }
#endif //RWDTS_XACT_REF_DEBUG_ENABLE
...
static const GEnumValue  _rwdts_xact_flag_values[] =  {
  ...
  {RWDTS_XACT_FLAG_KEYLESS,        "KEYLESS",    "KEYLESS"},
  { 0, NULL, NULL}
};
                         ...
                         RWPB_T_MSG(RwDts_data_Dts_Member_State_Registration_CacheEntry)  **cache_entry)
{
  ...
  if (cache_entry)
  {
    if (entry)
    {
      ...
      rwdts_member_data_delete_cursors(NULL, (rwdts_member_reg_handle_t)entry);
    }
  }
  return num_entries;
}
                             ...
                             void*                    getnext_ptr)
{
  ...
  if (!inp || (inp && !inp->state) || (inp && inp->state && inp->state->registration) ||
    ...
    while (entry) {
      ...
      mbr_state.registration[i]->has_id = 1;
      ...
      mbr_state.registration[i]->has_flags = 1;
      ...
      if (xact_entry->queries) {
        ...
        HASH_ITER(hh, xact_entry->queries, xquery, qtmp) {
          ...
          if (xquery->query) {
            ...
            mbr_state.transaction[i]->queries[j]->has_query_id = 1;
          }
          ...
          if (mbr_state.transaction[i]->queries[j]->n_match_reg) {
            ...
            while (match) {
              ...
              mbr_state.transaction[i]->queries[j]->match_reg[k]->has_reg_id = 1;
              k++;
    ...
    HASH_ITER(hh, apih->xacts, xact_entry, tmp_xact_entry) {
      if (!strcmp(inp->xact_trace[0]->xact_id, rwdts_xact_id_str(&xact_entry->id, xact_id_str, sizeof(xact_id_str)))) {
        ...
        for (i=0; i < xact_entry->curr_trace_index; i++) {
           ...
           xact_trace.trace_result[i]->has_id = 1;
...
static void rwdts_conf1_apply(rwdts_api_t *apih,
  ...
  rwdts_member_cursor_t *cur = rwdts_member_data_get_cursor(xact, apih->conf1.tracert.reg);
  ...
  while ((tr = (RWPB_T_MSG(RwDts_Tracert)*) rwdts_member_reg_handle_get_next_element(apih->conf1.tracert.reg,
    ...
    if (tr->keyspec) {
      ...
      if (ks->has_print_) {
        apih->conf1.tracert.filters[apih->conf1.tracert.filters_ct]->print = ks->print_;
      }
      if (ks->break_at) {
  ...
  rwdts_member_data_delete_cursors(xact, apih->conf1.tracert.reg);
  return;
...
void
rwdts_xact_deinit_cursor(rwdts_xact_t *xact)
{
  int count;
...
{
  rwdts_api_t  *apih = rwdts_api_config_ready_handle->apih;
  rwdts_api_config_ready_t *component;
  RW_SKLIST_FOREACH(component, rwdts_api_config_ready_t, &(rwdts_api_config_ready_handle->components), config_ready_elem) {
    fprintf(stderr, "%s:Component %s is in state %d\n",
            apih->client_path,
...
rwdts_api_member_send_xact_error(rwdts_api_t*                    apih,
                                 ...
                                 char *                          errstr)
{
  rwdts_member_send_error(xact, NULL, NULL, apih, NULL, status, errstr);
}

rwvcs_instance_ptr_t 
rwdts_api_get_vcs_inst(rwdts_api_t *apih)
{
  return (apih->rwtasklet_info? apih->rwtasklet_info->rwvcs: NULL);
}
...
rwdts_api_get_instance_name(rwdts_api_t *apih)
{
  ...
  char *instance_name = 
      to_instance_name(apih->rwtasklet_info->identity.rwtasklet_name, apih->rwtasklet_info->identity.rwtasklet_instance_id);
...
{
  return(apih->ha_uagent_state_regh);
}
...
rwdts_api_set_uagent_state_regh(rwdts_api_t *apih,
                                rwdts_member_reg_handle_t regh)
{
  apih->ha_uagent_state_regh = regh;
}
