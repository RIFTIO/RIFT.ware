                                        ...
                                        rw_keyspec_path_t**            keyspec)
{
  ...
  rwdts_member_data_object_t*  xact_obj = NULL;
  ...
  if (cursor->xact) {
    ...
    if (reg->flags & RWDTS_FLAG_PUBLISHER) {
      xact_obj = xact->pub_obj_list;
    } else {
      xact_obj = xact->sub_obj_list;
    }
  } else {
  ...
  if (HASH_CNT(hh_data,reg->obj_list) > cursor->position) {
	...
	look_obj = NULL;
        if (cursor->xact != NULL) {
          HASH_FIND(hh_data, xact_obj, data->key, data->key_len, look_obj);
        }
        if (look_obj) {
  ...
  if (cursor->xact) {
    HASH_ITER(hh_data, xact_obj, data, tmpdata) {
      if (pos++ == cursor->position) {
        cursor->position++;
        RW_ASSERT(data->keyspec);
                                   ...
                                   const ProtobufCMessage**  out_msg)
{
  ...
  if (xact != NULL) {
    ...
    if (reg->flags & RWDTS_FLAG_PUBLISHER) { 
      obj_list_p = &xact->pub_obj_list;
    } else {
      obj_list_p = &xact->sub_obj_list;
    }
  } else {
                            ...
                            rwdts_member_reg_handle_t  regh)
{
  ...
  if (xact != NULL) {
    if (reg->flags & RWDTS_FLAG_PUBLISHER) {
      return (HASH_CNT(hh_data, xact->pub_obj_list));
    } else {
      return (HASH_CNT(hh_data, xact->sub_obj_list));
    }
  }
...
rwdts_member_reg_handle_get_cursor(rwdts_member_reg_handle_t regh,
                                   rwdts_xact_t*             xact)
{
  return rwdts_member_data_get_cursor(xact, regh);
}

/*
 * Free the cursor(s) allocated for xact and reg
 */
void
