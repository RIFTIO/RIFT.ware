...
protected:
  static void SetUpTestCase() {
    ...
    tenv.rserver_path = RW_STRDUP_PRINTF("%s/usr/bin/redis-server", rw_install_dir); 
...
rwdts_redis_m_to_s_cb(rwdts_redis_msg_handle*, void *userdata)
{
  ...
  rwsched_dispatch_main_until(tenv->redis_tasklet, 3/*s*/, NULL);
...
TEST_F(RWDtsRedisNCEnsemble, TestMasterSlave)
{
  ...
  rwsched_dispatch_main_until(tenv.redis_tasklet, 8/*s*/, NULL);
}
