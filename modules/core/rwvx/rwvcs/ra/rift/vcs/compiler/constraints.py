...
class UniqueUAgent(SystemConstraint):
    def __call__(self, sysinfo):
        ...
        a_uagent, stdby_uagent, stdby2_uagent = None, None, None
        for uagent in uagents:
            ...
            if not uagent.mode_active:
                if not stdby_uagent:
                    stdby_uagent = uagent
                elif not stdby2_uagent:
                    stdby2_uagent = uagent
                else:
                    raise exc.ConstraintError('more than two standby uAgent')

        if stdby_uagent: uagents.remove(stdby_uagent)
        if stdby2_uagent: uagents.remove(stdby2_uagent)
