...
static rw_status_t initialize_zookeeper(rwvcs_instance_ptr_t rwvcs, const char *ip_address, const char *rift_var_root)
{
  rw_status_t status;
  ...
  status = start_zookeeper_server(rwvcs, rwvcs->pb_rwmanifest->bootstrap_phase, rift_var_root);
  RW_ASSERT(status == RW_STATUS_SUCCESS);
  if (status != RW_STATUS_SUCCESS)
    return status;
...
static char* rwvcs_get_rift_var_root()
{
...
rwvx_instance_alloc(void)
{
  ...
  rwvx->rwvcs_zk_module = rwvcs_zk_module_alloc();
  RW_ASSERT(rwvx->rwvcs_zk_module);
...
void rwvx_instance_free(rwvx_instance_t * rwvx)
{
  ...
  rwvcs_zk_module_free(&(rwvx->rwvcs_zk_module));
}
    ...
    int (*rwmain_f)(int argc, char ** argv, char ** envp))
{
  ...
  char *rift_var_root = rwvcs_get_rift_var_root();
  RW_ASSERT(rift_var_root);

  rw_util_create_rift_var_root((const char *)rift_var_root);   

  status = initialize_zookeeper(rwvcs, ip_address, rift_var_root);
