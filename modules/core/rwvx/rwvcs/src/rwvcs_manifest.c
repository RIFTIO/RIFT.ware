    ...
    const char           *ip_address)
{
  ...
  for (indx = 0; (indx < RWVCS_ZK_SERVER_CLUSTER) && bootstrap->ip_addrs_list && store_indx<bootstrap->n_ip_addrs_list; indx++) {
    ...
    mgmt_info->zk_server_port_details[indx] = rwvcs_zk_server_port_detail_alloc(
        zk_server_addr,
        zk_client_port,
        indx+1,
        zk_server_start,
