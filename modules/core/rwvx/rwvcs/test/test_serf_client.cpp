...
static serf_context_ptr_t start_serf(rwvcs_instance_ptr_t rwvcs, pid_t * pid)
{
  ...
  r = asprintf(&serf_path, "%s", "/usr/bin/serf");
  if (r==-1) {
    return NULL;
  }
...
class TestSerfRPC: public ::testing::Test {
  ...
  protected:
    virtual void SetUp() {
      ...
      m_rwvx->rwlog = rwlog_init("Logging");
...
TEST_F(TestSerfRPC, DISABLED_TestStartup) {
  TestStartup();
}

TEST_F(TestSerfRPC, DISABLED_TestEventCallback) {
  TestEventCallback();
}
