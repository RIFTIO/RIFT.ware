...
class RwvcsRwzkTest: public ::testing::Test {
  ...
  protected:
    ...
    virtual void SetUp() {
      ...
      status = rwvcs_zk_zake_init(RWVX_GET_ZK_MODULE(m_rwvx));
      RW_ASSERT(status == RW_STATUS_SUCCESS);

      status = rwvcs_rwzk_seed_auto_instance(m_rwvcs, 100, NULL);
      RW_ASSERT(status == RW_STATUS_SUCCESS);

      status = rwvcs_zk_create(RWVX_GET_ZK_MODULE(m_rwvx), "/rwvcs", NULL);
      RW_ASSERT(status == RW_STATUS_SUCCESS);
    }
    ...
    virtual void TearDown() {
      ...
      rwvcs_zk_get_children(RWVX_GET_ZK_MODULE(m_rwvx), "/rwvcs", &children, NULL);
      for (int i = 0; children[i]; ++i)
        rwvcs_rwzk_delete_node(m_rwvcs, children[i]);

      rwvcs_zk_delete(RWVX_GET_ZK_MODULE(m_rwvx), "/rwvcs", NULL);
    ...
    virtual void TestLockTest() {
      ...
      for (i = 0; i < RWVCS_RWZK_MAX_ENTRY; i ++) {
        rwsched_dispatch_queue_release(tasklet, rwq[i]);
      }
      for (i = 1; i < RWVCS_RWZK_MAX_ITER; i++) {
