...
typedef struct rwvcs_master_vm_info_s {
  char vm_name[RWVCS_VM_NAME_LEN];
  struct in_addr vm_addr;
} rwvcs_master_vm_info_t;

typedef struct rwdts_member_reg_handle_s* rwdts_member_reg_handle_t;
typedef struct rwmain_restart_instance_s  rwmain_restart_instance_t;
...
typedef struct rwvcs_mgmt_info_s {
  rwvcs_zk_server_port_detail_ptr_t zk_server_port_details[RWVCS_ZK_SERVER_CLUSTER + 1]; // for NULL terminated list
  uint32_t  unique_ports:1;
  uint32_t  mgmt_vm:1;
  uint32_t  config_start_zk_pending:1;
  vcs_vm_state state;
  rwvcs_master_vm_info_t master_vm_info;
