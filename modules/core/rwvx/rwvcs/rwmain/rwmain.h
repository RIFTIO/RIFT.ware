...
/*
 * Debug function to skip reading heartbeat messages from the mq
 * For unit test ussage only. This will eventually cause the restart
 *
 * @param rwmain        - rwmain instance
 * @return              - RW_STATUS_SUCCESS/FAIL
 */
...
//#define RWVCS_NO_STBY_UAGENT_SPRT
...
#define RWMAIN_SKIP_COMPONENTS(t_cinfo, t_failed_vm) \
    (!strcmp((t_cinfo).component_name, "dtsrouter") \
     ...
     || strstr((t_cinfo).instance_name, "RW.Redis.Server") \
...
#define RWMAIN_SKIP_COMPONENTS(t_cinfo, t_failed_vm) \
    (!strcmp((t_cinfo).component_name, "dtsrouter") \
     ...
     ||!strcmp((t_cinfo).component_name, "RW.TermIO") \
     || strstr((t_cinfo).instance_name, "RW.Redis.Server") \
     ||strstr((t_cinfo).instance_name, "uAgent") \
     ||strstr((t_cinfo).instance_name, "confd") \
     ||strstr((t_cinfo).instance_name, t_failed_vm))
#endif
