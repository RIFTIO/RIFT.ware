...
def create_server_config(myid, client_port, unique_ports, server_details, rift_var_root=None):
    """
    ...
    if rift_var_root is not None:
      os.environ['RIFT_VAR_ROOT'] = rift_var_root
      rift_var_root_dir = rift_var_root
    else:
      rift_var_root_dir = os.environ.get('INSTALLDIR', '')

    config_fname = "%s/etc/zookeeper/server-%d.cfg" % (install_dir, myid)

    config = "tickTime=2000\n"
    config += "dataDir=%s/zk/server-%d/data\n" % (rift_var_root_dir, myid)
    config += "clientPort=%d\n" % (client_port)
    config += "initLimit=5\n"
    config += "syncLimit=2\n"
    ...
    try:
        chk_dir = os.stat("%s/zk/server-%d/data/" % (rift_var_root_dir, myid))
    except FileNotFoundError:
        try:
            chk_dir = os.stat("%s/zk/server-%d/" % (rift_var_root_dir, myid))
            os.mkdir("%s/zk/server-%d/data/" % (rift_var_root_dir, myid))
        except FileNotFoundError:
            try:
                chk_dir = os.stat("%s/zk/" % (rift_var_root_dir))
                os.mkdir("%s/zk/server-%d/" % (rift_var_root_dir, myid))
                os.mkdir("%s/zk/server-%d/data/" % (rift_var_root_dir, myid))
            except FileNotFoundError:
                os.mkdir("%s/zk/" % (rift_var_root_dir))
                os.mkdir("%s/zk/server-%d/" % (rift_var_root_dir, myid))
                os.mkdir("%s/zk/server-%d/data/" % (rift_var_root_dir, myid))

    id_fname = "%s/zk/server-%d/data/myid" % (rift_var_root_dir, myid)
...
def server_start(myid):
    ...
    platform = os.environ.get('RIFT_PLATFORM')
    ...
    if platform == "ub16":
        classpath = ':'.join([
            ...
            "/usr/share/java/netty-3.9.0.Final.jar.jar",
...
log4j.appender.ZLOG.File=""" + to_java_compatible_path(  # NOQA
                "%s/zk/server-%d/log" % (rift_var_root, myid) + os.sep + "zookeeper.log\n"))
    ...
    argv = [
        ...
        classpath,
        #'-Dlog4j.debug',
        '-Dzookeeper.log.dir="%s"' % (rift_var_root,),
