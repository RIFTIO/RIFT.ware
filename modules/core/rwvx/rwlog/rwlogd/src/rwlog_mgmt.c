                        ...
                        RWPB_T_MSG(RwlogMgmt_data_Logging_Sink) *Sink)
{
  ...
  if (Sink && Sink->filter) {
    status = rwlog_mgmt_config_filter(instance, Sink->name, (void *)Sink->filter);
  }
  return status;
}
                          ...
                          RWPB_T_MSG(RwlogMgmt_data_Logging_Console) *Console)
{
  ...
  if(Console->filter) {
    status = rwlog_mgmt_config_filter(instance, RWLOGD_CONSOLE_SINK_NAME, (RWPB_T_MSG(RwlogMgmt_data_Logging_Sink_Filter) *)Console->filter);
  }
  return status;
}
...

{
  ...
  switch (action) {
    ...
    case RWDTS_QUERY_UPDATE:
      if (Config->n_sink) {
        rw_status_t rs = rwlog_mgmt_config_sink(instance, Config->n_sink, Config->sink);
      ...
      if(Config->console) {
        rw_status_t rs = rwlog_mgmt_config_console(instance, Config->console);
        if (rs != RW_STATUS_SUCCESS) {
          return RWDTS_ACTION_NOT_OK;
        }
      }
                                   ...
                                   void *getnext_ptr)
{
  ...
  if(log_action->filter && (log_action->filter->has_next_call || log_action->filter->has_next_failed_call)) {
    ...
    rw_status_t rs = rwlog_mgmt_rpc_config_filter(instance, name, log_action->filter);
    if (rs != RW_STATUS_SUCCESS) {
      return RWDTS_ACTION_NOT_OK;
    }
  }
