...
static void init_broker(rwmsg_perf_task_t *t) {
  ...
  *(pnum-1) = '\0'; // to extract host-ip from  "tcp://<host-ip>:<port>
  ...
  //t->bro = rwmsg_broker_create(0, t->cfg.instance, "127.0.0.1", perf.rwsched, t->tasklet, perf.rwvcs_zk, t->cfg.mainq, t->ep);
  t->bro = rwmsg_broker_create(0, t->cfg.instance, pnum, perf.rwsched, t->tasklet, perf.rwvcs_zk, t->cfg.mainq, t->ep, perf.ti);
  perf.bros_ct++;
}
...
static void deinit_tasklet(rwmsg_perf_task_t *t) {
  ...
  if (!t->cfg.mainq) {
    rwsched_dispatch_queue_release(t->tasklet, t->rws_q);
  }
  t->rws_q = NULL;
...
void start_zookeeper() {
  ...
  for (it = ifap; it != NULL; it = it->ifa_next) {
    ...
    if (!strncmp(master_ip, inet_ntoa(addr->sin_addr), 15)) {
      ...
      rwvcs_zk_server_port_detail_ptr_t zk_server_port_detail = rwvcs_zk_server_port_detail_alloc(
          server_name,
          2181,
          1,
          true,
      ...
      rwvcs_zk_server_port_detail_ptr_t zk_server_port_details[] = {
        zk_server_port_detail,
        NULL
      };
      status = rwvcs_zk_create_server_config(perf.rwvcs_zk, getpid(), 2181, 0, zk_server_port_details, NULL);
      RW_ASSERT(status == RW_STATUS_SUCCESS);
      int ret_pid = rwvcs_zk_server_start(perf.rwvcs_zk,getpid());
      RW_ASSERT(ret_pid > 0);
...
int main(int argc, char * const *argv, const char *envp[]) {
  ...
  perf.rwvcs_zk = RWVX_GET_ZK_MODULE(perf.ti->rwvx);
  RW_ASSERT(perf.rwvcs_zk);

#if 1 // ZOOKEEPER
  if (perf.zookeep) {
    start_zookeeper();
    RwVcsZkServerPortDetail server_detail;
    server_detail.zk_client_port = 2181;
    server_detail.zk_server_addr = "127.0.0.1";
    server_detail.zk_server_id = 1;
    rwvcs_zk_server_port_detail_ptr_t server_details[2] = { &server_detail, NULL };
    status = rwvcs_zk_kazoo_init(perf.rwvcs_zk, server_details);
    RW_ASSERT(RW_STATUS_SUCCESS == status);
  } else {
    status = rwvcs_zk_zake_init(perf.rwvcs_zk);
    RW_ASSERT(RW_STATUS_SUCCESS == status);
  }
#endif
