...
/* Code as run for chantype BROSRV from original borked code running in the takeover channel's context */
static void rwmsg_broker_srvchan_bindings_flush_1(void *sc_in) {
  rwmsg_broker_srvchan_t *sc_o = (rwmsg_broker_srvchan_t*)sc_in;

  /* What is this code doing?  Seems to be flushing / bouncing the
     whole ingress queues for the formerly bound brosrv channel, which
     the calling sc is taking over for on this method? */

  int p;
  for (p=RWMSG_PRIORITY_COUNT-2; p>=0; p--) {
    rwmsg_request_t *req_o = NULL;
    while ((req_o = rwmsg_queue_dequeue_pri(&sc_o->ch.localq, p))) {
      /* BUG: how would this be true? */
      RW_ASSERT(!req_o);
      /* BUG: req_o is leaked?  shouldn't it be requeued or something something something? */
    }
	...
	if (!rwmsg_broker_srvchan_recv_req(sc_o, qreq)) {   // this sends the bounces
	  qreq = NULL;
	}
...
/* Handle refct outside of logic in flush_1() */
static void rwmsg_broker_srvchan_bindings_flush_1_f(void *sc_in) {
  rwmsg_broker_srvchan_bindings_flush_1(sc_in);
  rwmsg_broker_srvchan_release((rwmsg_broker_srvchan_t*)sc_in);
}
...
static void rwmsg_broker_srvchan_msgctl(rwmsg_broker_srvchan_t *sc,
  ...
  case RWMSG_MSGCTL_METHNO_METHBIND:
	...
	rwmsg_broker_t *bro = sc->bch.bro;
	ch = rwmsg_endpoint_find_channel(sc->ch.ep, RWMSG_METHB_TYPE_BROSRVCHAN, bro->bro_instid, mb->k.pathhash, mb->k.payt, mb->k.methno);
	if (ch) {
	  /* This method is already bound to some channel */
	  if (ch == (rwmsg_channel_t*)sc) {
            /* It is us already. */
	    goto ret;
	  } else {
	    /* It is not us, so we take over. */
            rwmsg_broker_srvchan_t *sc_o = (rwmsg_broker_srvchan_t*)ch;
	    if (sc_o->ch.halted) {
	      /* Msgctl taking over from a halted channel.  This should be fine or even expected (??) */
	    }
	    switch (sc_o->ch.chantype) {
	    case RWMSG_CHAN_PEERSRV:
        if (strstr(mb->path, "RW.uAgent") || strstr(mb->path, "RW.CLI")) {
          goto ret;
        }
        // FALLTHROUGH
	    case RWMSG_CHAN_BROSRV:
          ...
          if (sc_o->ch.chantype == RWMSG_CHAN_BROSRV) {
		        RWMSG_TRACE_CHAN(&sc->ch, NOTICE, "msgctl add methbinding duplicate found on CHAN_BROSRV, "
                            "treating as service-recreate; chanid=%u already registered path='%s' phash=0x%lx methno=%u payt=%d",
				                    ch->chanid,
				                    mb->path,
				                    mb->k.pathhash,
				                    mb->k.methno,
		      ...
		      /* Delete all the old bindings in the endpoint */
		      rwmsg_endpoint_del_channel_method_bindings(sc_o->ch.ep, ch);

		      /* Find the right scheduler mode and queue to dispatch
		        this onto for the found channel to run it.  Note
		        that we hold a ref on this target channel (from the
		        find_channle lookup) until end of this function
		        where we release it. */
		      rwsched_dispatch_queue_t rwq = NULL;
		      switch(ch->notify.theme) {
		      case RWMSG_NOTIFY_RWSCHED:
		        rwq = ch->notify.rwsched.rwqueue;
		        RW_ASSERT(rwq);
		        break;
		      case RWMSG_NOTIFY_CFRUNLOOP:
		        rwq = rwsched_dispatch_get_main_queue(bro->ep->rwsched);
		        RW_ASSERT(rwq);
		        break;
		      default:
		      ...
		      _RWMSG_CH_DEBUG_(&sc_o->ch, "++");
		      ck_pr_inc_32(&sc_o->ch.refct); /* bump refct on target channel for rwq transit etc, released in flush_1_f() */
		      rwsched_dispatch_async_f(bro->tinfo, rwq, (void*)sc_o, rwmsg_broker_srvchan_bindings_flush_1_f);


		      /* The above flush logic really only flushes.  DTS has
		        bodges to deal with that, however we would really
		        prefer bidning move transparency at the rwmsg
		        layer. That would involve some slightly fiddly
		        requeue logic across channels. */
	      }
	      break;
	...
	/* Unit test exit hint(s) */
	ck_pr_inc_32(&rwmsg_broker_g.exitnow.method_bindings);

	/* Gadzooks why isn't this a function */
...

	{
	  ...
	  /*
	   * We have to dispatch to acceptor's thread, because we are iterating
