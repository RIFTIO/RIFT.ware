...
static void haltneg(struct rwmsg_broker_acceptor_negotiating_s *neg) {
  ...
  RW_ASSERT(neg->bro);
  rwmsg_garbage(&neg->bro->ep->gc, RWMSG_OBJTYPE_RWSCHED_SRCREL, neg->rwstimer, neg->bro->ep->rwsched, neg->taskletinfo);
  neg->rwstimer = NULL;
  rwmsg_garbage(&neg->bro->ep->gc, RWMSG_OBJTYPE_RWSCHED_SRCREL, neg->rwssocket, neg->bro->ep->rwsched, neg->taskletinfo);
  neg->rwssocket = NULL;
