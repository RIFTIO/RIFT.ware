    ...
    */
    ...
    if (tv_begin.tv_sec
      ...
      if (cbms >= sched_tasklet->instance->latency.check_threshold_ms * 5) {
        char *name = rw_unw_get_proc_name(rwsched_object->cf_callout);
        RWSCHED_LOG_EVENT(instance, SchedError, 
                          RWLOG_ATTR_SPRINTF("rwsched[%d] CF timer took %ums ctx %p callback %s",
				   ...
				   void *info)
{
  ...
  if (sched_tasklet->blocking_mode.cfsource == callback_context->cfsource) {
	...
	&& sched_tasklet->instance->latency.check_threshold_ms) {
      ...
      unsigned int cbms = (tv_delta.tv_sec * 1000 + tv_delta.tv_usec / 1000);
      if (cbms >= sched_tasklet->instance->latency.check_threshold_ms * 5) {
	      char *name = rw_unw_get_proc_name(callback_context->cf_callout);
        RWSCHED_LOG_EVENT(instance, SchedError, 
          RWLOG_ATTR_SPRINTF("rwsched[%d] CF socket/file took %ums ctx %p callback %s", 
