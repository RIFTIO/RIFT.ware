...
void rwdynschema_load_all_schema(void * context)
{
  ...
  auto rift_install_dir = rwyangutil::get_rift_install(); 
  auto rift_var_root = rwyangutil::get_rift_var_root();
  RW_ASSERT (rift_var_root.length());

  fs::path latest_version_directory(rift_var_root + "/" + RW_SCHEMA_VER_LATEST_PATH);
