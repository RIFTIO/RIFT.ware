...
{
  return static_cast<const rw_yang_pb_schema_t*>(static_cast<YangModel*>(model)->get_ypbc_schema());
}
...
YangNode* YangNode::search_child_rpcio_only(
    const char* name)
{
  RW_ASSERT(name);
  RW_ASSERT(name[0]);
  RW_ASSERT(get_stmt_type() == RW_YANG_STMT_TYPE_RPC);
...
YangNode* YangNode::search_child(
    const char* name,
    const char* ns)
{
  RW_ASSERT(name);
...
void YangNode::set_mode_path_to_root()
{
  // Unsupported by default.
  // ATTN: Should have a way to return an error?
}
...
{
  return static_cast<rw_yang_model_t*>(static_cast<YangNode*>(ynode)->get_model());
}
  ...
  const rw_yang_pb_module_t *ypbc_module)
{
  ...
  if (ypbc_module->rpci_module) {
    for (size_t i = 0; i < ypbc_module->rpci_module->child_msg_count; i++) {
      ...
      RW_ASSERT(yn);
      YangNode *rpc_input = yn->search_child_rpcio_only("input");
      RW_ASSERT(rpc_input);
  ...
  if (ypbc_module->rpco_module) {
    for (size_t i = 0; i < ypbc_module->rpco_module->child_msg_count; i++) {
      ...
      RW_ASSERT(yn);
      YangNode *rpc_output = yn->search_child_rpcio_only("output");
