...
bool PbModel::traverse(PbModelVisitor* visitor)
{
  bool success = true;
  if (has_data_) {
    for (const auto& pbmod: modules_by_dependencies_) {
      if (!pbmod->is_local() || !pbmod->has_schema()) {
        continue;
      }
      success = visitor->visit_module_enter(pbmod);
      if (!success) {
        break;
      }
      success = pbmod->traverse(visitor);
      if (!success) {
        break;
      }
      success = visitor->visit_module_exit(pbmod);
    }
  }
  return success;
}
  ...
  if (basename) {
    ...
    switch (file_type) {
      ...
      case doc_file_t::HTML: {
        ...
        HTMLDocWriter writer(filename, HTMLDocType::DOC_TYPE_USER);
        if (!writer.is_good()) {
          error(filename, "Unable to open file");
          return parser_control_t::DO_SKIP_TO_SIBLING;
        }
  ...
  if (basename) {
    ...
    switch (file_type) {
      ...
      case doc_file_t::HTML: {
        ...
        HTMLDocWriter writer(filename, HTMLDocType::DOC_TYPE_API);
        if (!writer.is_good()) {
          error(filename, "Unable to open file");
          return parser_control_t::DO_SKIP_TO_SIBLING;
        }
