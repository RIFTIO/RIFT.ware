  ...
  const char* type)
{
  ...
  for (auto ni = pbnode_->child_begin(); ni != pbnode_->child_end(); ++ni) {
    ...
    if (stmt_type == RW_YANG_STMT_TYPE_RPC) {
      ...
      RW_ASSERT(rpc_ynode);
      YangNode* type_ynode = rpc_ynode->search_child_rpcio_only(type);
...
bool PbMessage::traverse(PbModelVisitor* visitor)
{
  for (auto fi = field_list_.begin(); fi != field_list_.end(); ++fi) {
    PbMessage* field_pbmsg = (*fi)->get_field_pbmsg();
    if (!field_pbmsg) {
    ...
    if (!visitor->visit_message_enter(field_pbmsg)) {
      return false;
    }
    if (!field_pbmsg->traverse(visitor)) {
      return false;
    }
    if (!visitor->visit_message_exit(field_pbmsg)) {
      return false;
    }
  }
  return true;
