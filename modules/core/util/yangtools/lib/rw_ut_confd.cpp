...
: test_name_(test_name)
{
  ...
  if (nullptr == rift_install) {
    cerr << "Unable to find $RIFT_INSTALL" << std::endl;
    throw std::exception();
  }
  rift_install_ = rift_install;


  // Verify confd root exists
  confd_install_root_ = rift_install_/"usr/local/confd";
  ...
  test_root_ = rift_install_/"tmp/confd"/test_name_;
  ...
  YangModel* yang_model)
{
  try {
    ...
    FS::path search_paths[] = {
      FS::path{cwd_},
      FS::path{cwd_/"yang"},
      FS::path{rift_install_/"usr/data/yang"/test_name_},
      FS::path{rift_install_/"usr/data/yang/unittest"},
      FS::path{rift_install_/"usr/data/yang"},
    };
...
void ConfdUnittestHarness::start()
{
  ...
  FS::path confd_exe = rift_install_/"usr/local/confd/bin/confd";
