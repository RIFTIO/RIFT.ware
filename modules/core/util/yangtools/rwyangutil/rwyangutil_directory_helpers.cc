...
: rift_install(get_rift_install()),
  rift_var_root(get_rift_var_root()),
  install_yang_dir     ( rift_install  + "/" + RW_INSTALL_YANG_PATH ),
  install_manifest_dir ( rift_install  + "/" + RW_INSTALL_MANIFEST_PATH ),
  schema_root_dir      ( rift_var_root + "/" + RW_SCHEMA_ROOT_PATH ),
  lock_dir             ( rift_var_root + "/" + RW_SCHEMA_LOCK_PATH ),
  lock_file            ( rift_var_root + "/" + RW_SCHEMA_LOCK_PATH + "/" + RW_SCHEMA_LOCK_FILENAME ),
  tmp_dir              ( rift_var_root + "/" + RW_SCHEMA_TMP_PATH ),
  version_dir          ( rift_var_root + "/" + RW_SCHEMA_VER_PATH ),
  version_latest_link  ( rift_var_root + "/" + RW_SCHEMA_VER_LATEST_PATH ),
  schema_all_tmp       ( rift_var_root + "/" + RW_SCHEMA_TMP_ALL_PATH ),
  schema_northbound_tmp( rift_var_root + "/" + RW_SCHEMA_TMP_NB_PATH )
{
}
...
bool create_schema_directory(std::vector<std::string> const & nb_listings)
{
  ...
  random_directory << paths.rift_var_root << "/" << RW_SCHEMA_INIT_TMP_PREFIX << suuid;
  ...
  std::string const initial_version_path = paths.rift_var_root + "/" + RW_SCHEMA_VER_PATH + "/00000000";
...
bool archive_mgmt_persist_workspace(const char* prefix, std::string path_name)
{
  for (fs::directory_iterator entry(path_name);
       entry != fs::directory_iterator(); ++entry)
  {
    ...
    auto new_dir_name = path_name + "/ar_" + dir_name + "_" + std::to_string(epoch);
...
bool remove_mgmt_workspace(const char* prefix, std::string path_name)
{
  for (fs::directory_iterator entry(path_name);
       entry != fs::directory_iterator(); ++entry)
  {
...
bool remove_unique_mgmt_workspace(std::string dir_name)
{
  ...
  auto ret1 = remove_mgmt_workspace(RW_SCHEMA_MGMT_TEST_PREFIX, dir_name);
  auto ret2 = remove_mgmt_workspace(RW_SCHEMA_MGMT_ARCHIVE_PREFIX, std::move(dir_name));
...
{
  return remove_mgmt_workspace(RW_SCHEMA_MGMT_PERSIST_PREFIX, std::move(dir_name));
}

bool archive_mgmt_persist_workspace(std::string dir_name)
{
  return archive_mgmt_persist_workspace(RW_SCHEMA_MGMT_PERSIST_PREFIX, std::move(dir_name));
}
