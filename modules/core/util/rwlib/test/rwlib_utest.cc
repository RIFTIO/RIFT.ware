...
TEST(RIFTMisc, CRC16_CCITT_ZERO) {
  TEST_DESCRIPTION("Test the rw_crc16_ccitt_zero C wrapper in rwlib");
  struct t {
    const char *buf;
    int len;
    uint16_t crc;
...
#if 0
    /* Test data for real crc16-ccitt from http://srecord.sourceforge.net/crc16-ccitt.html */
    { "", 0, 0x1d0f },
    { "A", 1, 0x9479 },
    { "123456789", 9, 0xe5cc },
    { "", 256, 0xe938 },
#endif

    /* Values from the crc16 embedded inside redis, itself copied from
       http://www.menie.org/georges/embedded/crc16.html */
    { "", 0, 0x0000 },
    { "A", 1, 0x58e5 },
    { "123456789", 9, 0x31c3 },
    { "", 256, 0xabe3 },
    { "argle-bargle", 12, 0x54e1 },
    { "fji0rt4ngu40gnu4g5j20@%^(*&^%$#@", 32, 0x8766 },

    { NULL, 0, 0 }
  };
  if (getenv("V")) {
    for (struct t *input = &tests[0];
	 input->buf;
	 input++) {
      printf("crc16 test[%d] expected %04x rw_crc16()=%04x\n",
	     (int)(input - &tests[0]),
	     (int)input->crc,
	     (int)(unsigned)rw_crc16_ccitt_zero(input->buf, input->len));
    }
  }
  for (struct t *input = &tests[0];
       input->buf;
       input++) {
    /* ASSERT_EQ(crc16_ccitt(input->buf, input->len), input->crc); */    // borrow from redis code to verify/generate test data
    ASSERT_EQ(rw_crc16_ccitt_zero(input->buf, input->len), input->crc);
  }
}
