...
'''
start vnf ipsec vnf-name cag vnf-instance 0 service name client tunnel-clones name fake count 5 starting-from 1
stop vnf ipsec vnf-name cag vnf-instance 0 service name client ike-clones name fake count 5 starting-from 1 ike-name name ike2bc
'''
...
sleep 2
if {{ $trigger eq "start" }} {{
  send "start vnf ipsec vnf-name cag vnf-instance 0 service name client tunnel-clones name fake count {tun_count} starting-from 1 \r"
}} else {{
  send "stop vnf ipsec vnf-name cag vnf-instance 0 service name client ike-clones name fake count {tun_count} starting-from 1 ike-name name ike2bc \r"  
}}
expect "rift# "
exp_close -i $spid
...
def main(argv=sys.argv[1:]):
    ...
    try:
        ...
        ikec_vnfr = find_vnfr(yaml_cfg['vnfr_data_map'], 1)
