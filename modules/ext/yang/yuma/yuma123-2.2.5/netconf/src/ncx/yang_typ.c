...
static status_t 
    resolve_type (struct ncx_instance_t_ *instance,
                  yang_pcb_t *pcb,
                  tk_chain_t *tkc,
                  ncx_module_t *mod,
                  typ_def_t *typdef,
...
*********************************************************************/
static status_t 
    loop_test (ncx_instance_t *instance, typ_def_t *typdef)
{
    ...
    if (typdef->tclass == NCX_CL_NAMED) {
        testdef = typ_get_parent_typdef(instance, typdef);
        lastdef = testdef;
        while (testdef && res==NO_ERR) {
            if (testdef == typdef) {
                ...
                lastdef = testdef;    
                testdef = typ_get_parent_typdef(instance, testdef);
            }
        }
    }
...
*********************************************************************/
static status_t 
    one_restriction_test (ncx_instance_t *instance,
                      tk_chain_t *tkc,
                      ncx_module_t *mod,
                      typ_def_t *newdef)
{
    ...
    btyp = typ_get_basetype(instance, newdef);
    if (btyp == NCX_BT_UNION) {
        return NO_ERR;
    }

    rangeQ = typ_get_rangeQ_con(instance, newdef);
    strrest = typ_get_strrest(instance, newdef);
    
    /* check if proper base type restrictions
     * are present. Range allowed for numbers
     * and strings only
     */
    if (rangeQ && !dlq_empty(instance, rangeQ)) {
     ...
     */
    pat = typ_get_first_pattern(instance, newdef);
    if (pat) {
...
*********************************************************************/
static status_t 
    restriction_test (ncx_instance_t *instance,
                      tk_chain_t *tkc,
                      ncx_module_t *mod,
                      typ_def_t *typdef)
{
    ...
    if (typdef->tclass == NCX_CL_SIMPLE) {
        return one_restriction_test(instance, tkc, mod, typdef);
    ...
    while (testdef && testdef->tclass==NCX_CL_NAMED) {
        newdef = typ_get_new_named(instance, testdef);
        if (!newdef) {
            testdef = typ_get_parent_typdef(instance, testdef);
            continue;
        }
        ...
        testbtyp = typ_get_basetype(instance, newdef);
        if (testbtyp==NCX_BT_NONE) {
            newdef->def.simple.btyp = btyp;
            res = one_restriction_test(instance, tkc, mod, newdef);
            CHK_EXIT(res, retres);
...
*********************************************************************/
static status_t 
    consume_yang_rangedef (ncx_instance_t *instance,
                           tk_chain_t *tkc,
                           ncx_module_t *mod,
                           typ_def_t *typdef,
                           ncx_btype_t btyp)
{
    ...
    rangeQ = typ_get_rangeQ_con(instance, typdef);
    if (!rangeQ) {
    ...
    if (TK_CUR_STR(tkc)) {
        if (!xml_strcmp(instance, TK_CUR_VAL(tkc), YANG_K_MIN)) {
            /* flag lower bound min for later eval */
            rv->flags |= TYP_FL_LBMIN;
        } else if (!xml_strcmp(instance, TK_CUR_VAL(tkc), YANG_K_MAX)) {
            ...
            if (!xml_strcmp(instance, TK_CUR_VAL(tkc), YANG_K_NEGINF)) {
                /* flag lower bound -INF for later eval */
                rv->flags |= TYP_FL_LBINF;
            } else if (!xml_strcmp(instance, TK_CUR_VAL(tkc), YANG_K_POSINF)) {
                /* flag lower bound INF for later eval */
        ...
        if (btyp != NCX_BT_NONE) {
            if (btyp == NCX_BT_DECIMAL64) {
                res = ncx_convert_tkc_dec64(instance, 
                                            tkc, 
                                            typ_get_fraction_digits(instance, typdef), 
                                            &rv->lb);
            } else {
                res = ncx_convert_tkcnum(instance, tkc, btyp, &rv->lb);
            }
        } else {
            rv->lbstr = xml_strdup(instance, TK_CUR_VAL(tkc));
            if (!rv->lbstr) {
                res = ERR_INTERNAL_MEM;
            }
        }
    ...
    if (res != NO_ERR) {
        ...
        if (NEED_EXIT(res)) {
            typ_free_rangedef(instance, rv, btyp);
            return retres;
        }
    }
    ...
    case TK_TT_BAR:
        ...
        if (rv->flags & TYP_FL_LBMIN) {
            ...
            if (btyp != NCX_BT_NONE) {
                res = ncx_copy_num(instance, &rv->lb, &rv->ub, btyp);
            } else {
                rv->ubstr = xml_strdup(instance, rv->lbstr);
                if (!rv->ubstr) {
                    res = ERR_INTERNAL_MEM;
                }
            }
        ...
        if (TK_CUR_STR(tkc)) {
            ...
            if (btyp != NCX_BT_NONE) {
                if (btyp == NCX_BT_DECIMAL64) {
                    ...
                    res = ncx_convert_tkcnum(instance, tkc, btyp, &rv->ub);
                }
            } else {
                rv->ubstr = xml_strdup(instance, TK_CUR_VAL(tkc));
                if (!rv->ubstr) {
                    res = ERR_INTERNAL_MEM;
                }
            }
    ...
    if (!(rv->flags & TYP_RANGE_FLAGS)) {
        ...
        if (btyp != NCX_BT_NONE) {
            cmpval = ncx_compare_nums(instance, &rv->lb, &rv->ub, btyp);
            if (cmpval > 0) {
        ...
        if ((rv->flags & TYP_FL_LBINF) && !dlq_empty(instance, rangeQ)) {
...
*********************************************************************/
static status_t 
    consume_yang_range (ncx_instance_t *instance,
                        tk_chain_t *tkc,
                        ncx_module_t *mod,
                        typ_def_t *typdef,
                        boolean isrange)
{
    ...
    range = typ_get_range_con(instance, typdef);
    ...
    if (TK_CUR_STR(tkc)) {
        if (range->rangestr) {
            m__free(instance, range->rangestr);
        }
        range->rangestr = xml_strdup(instance, TK_CUR_VAL(tkc));
        if (!range->rangestr) {
            res = ERR_INTERNAL_MEM;
            ncx_print_errormsg(instance, tkc, mod, res);
            return res;
        ...
        res = tk_retokenize_cur_string(instance, tkc, mod);
        if (res != NO_ERR) {
            ncx_print_errormsg(instance, tkc, mod, res);
            return res;
        }           
    ...
    while (!done) {
        /* get one range spec */
        res = consume_yang_rangedef(instance, tkc, mod, typdef, rbtyp);
    ...
    if (TK_CUR_TYP(tkc)==TK_TT_LBRACE) {
        res = yang_consume_error_stmts(instance,  tkc, mod, &range->range_errinfo,
                                        &typdef->appinfoQ );
    }
...
*********************************************************************/
static status_t 
    consume_yang_pattern (ncx_instance_t *instance,
                            tk_chain_t *tkc,
                           ncx_module_t *mod,
                           typ_def_t *typdef )
{
    ...
    typ_set_strrest(instance, typdef, NCX_SR_PATTERN);
    ...
    if (TK_TT_MSTRING == TK_CUR_TYP(tkc) || 
        ...
        xmlChar *p, *buff = m__getMem(instance, TK_CUR_MODLEN(tkc) + 
                                      TK_CUR_LEN(tkc) + 2);
        ...
        p = buff;
        p += xml_strncpy(instance, p, TK_CUR_MOD(tkc), TK_CUR_MODLEN(tkc));
        *p++ = ':';
        p += xml_strncpy(instance, p, TK_CUR_VAL(tkc), TK_CUR_LEN(tkc));
        *p = 0;

        pat = typ_new_pattern(instance, buff);
        m__free(instance, buff);
    } else {
        pat = typ_new_pattern(instance, TK_CUR_VAL(tkc));
    }
    ...
    res = typ_compile_pattern(instance, pat);
    if (res != NO_ERR) {
        ncx_print_errormsg(instance, tkc, mod, res);
    ...
    dlq_enque(instance, pat, &typdef->def.simple.patternQ);
    ...
    case TK_TT_LBRACE:
        /* check sub-section for error-app-tag and error-message */
        res = yang_consume_error_stmts(instance,  tkc, mod, &pat->pat_errinfo,
                                        &typdef->appinfoQ);
        break;
    default:
        res = ERR_NCX_WRONG_TKTYPE;
        ncx_mod_exp_err(instance, tkc, mod, res, "semicolon or left brace" );
    }
...
*********************************************************************/
static status_t 
    finish_string_type (ncx_instance_t *instance,
                        tk_chain_t  *tkc,
                        ncx_module_t *mod,
                        typ_def_t *typdef,
                        ncx_btype_t btyp)
{
    ...
    while (!done) {
        ...
        case TK_TT_MSTRING:
            ...
            res = ncx_consume_appinfo(instance, tkc, mod, &typdef->appinfoQ);
            CHK_EXIT(res, retres);
            continue;
        case TK_TT_TSTRING:
        ...
        if (!xml_strcmp(instance, val, YANG_K_LENGTH)) {
            if (lendone) {
            ...
            lendone = TRUE;
            res = consume_yang_range(instance, tkc, mod, typdef, FALSE);
            CHK_EXIT(res, retres);
        } else if (!xml_strcmp(instance, val, YANG_K_PATTERN)) {
            ...
            res = consume_yang_pattern(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        } else {
...
*********************************************************************/
static status_t 
    finish_number_type (ncx_instance_t *instance,
                        tk_chain_t  *tkc,
                        ncx_module_t *mod,
                        ncx_btype_t btyp,
                        typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        /* Got a token string so check the value */
        if (!xml_strcmp(instance, val, YANG_K_RANGE)) {
            if (rangedone) {
                retres = ERR_NCX_ENTRY_EXISTS;
            ...
            rangedone = TRUE;
            res = consume_yang_range(instance, tkc, mod, typdef, TRUE);
            CHK_EXIT(res, retres);
        } else if (!xml_strcmp(instance, val, YANG_K_FRACTION_DIGITS)) {
            ...
            if (res == NO_ERR) {
                res = typ_set_fraction_digits(instance, typdef, (uint8)digits);
                if (res != NO_ERR) {
                    retres = res;
...
*********************************************************************/
static status_t 
    finish_unknown_type (ncx_instance_t *instance,
                         tk_chain_t  *tkc,
                         ncx_module_t *mod,
                         typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        if (!xml_strcmp(instance, val, YANG_K_RANGE)) {
            ...
            res = consume_yang_range(instance, tkc, mod, typdef, TRUE);
            CHK_EXIT(res, retres);
        } else if (!xml_strcmp(instance, val, YANG_K_LENGTH)) {
            if (rangedone) {
                retres = ERR_NCX_ENTRY_EXISTS;
                log_error(instance, "\nError: range clause already entered,"
...
*********************************************************************/
static status_t
    insert_yang_enumbit (ncx_instance_t *instance,
                         tk_chain_t *tkc,
                         ncx_module_t *mod,
                         typ_enum_t *en, 
                         typ_def_t *typdef,
                         ...
                         boolean valset)
{
    ...
    for (enl = (typ_enum_t *)dlq_firstEntry(instance, &sim->valQ);
         enl != NULL;
         enl = (typ_enum_t *)dlq_nextEntry(instance, enl)) {
        if (!xml_strcmp(instance, en->name, enl->name)) {
    ...
    if (valset) {
        ...
        if (dlq_empty(instance, &sim->valQ)) {
    ...
    dlq_enque(instance, en, &sim->valQ);
    return NO_ERR;
...
*********************************************************************/
static status_t 
    consume_yang_bit (ncx_instance_t *instance,
                      tk_chain_t *tkc,
                      ncx_module_t *mod,
                      typ_def_t *typdef)
{
     ...
     */
    res = yang_consume_id_string(instance, tkc, mod, &str);
    CHK_EXIT(res, retres);

    if (str) {
        enu = typ_new_enum2(instance, str);
    } else {
        enu = typ_new_enum(instance, (const xmlChar *)"none");
    }
    if (!enu) {
        res = ERR_INTERNAL_MEM;
        if (str) {
    ...
    while (!done) {
        ...
        case TK_TT_MSTRING:
            /* vendor-specific clause found instead */
            res = ncx_consume_appinfo(instance, tkc, mod, &enu->appinfoQ);
            if (res != NO_ERR) {
                retres = res;
                if (NEED_EXIT(res)) {
                    typ_free_enum(instance, enu);
        ...
        if (!xml_strcmp(instance, val, YANG_K_DESCRIPTION)) {
            ...
            res = yang_consume_status(instance, 
                                      ...
                                      &stat, 
                                      &enu->appinfoQ);
        } else if (!xml_strcmp(instance, val, YANG_K_POSITION)) {
            res = yang_consume_uint32(instance, 
                                      tkc, 
                                      mod, 
                                      &enu->pos,
    ...
    if (retres == NO_ERR) {
        res = insert_yang_enumbit(instance, 
                                  tkc, 
                                  mod, 
                                  enu, 
                                  typdef,
...
*********************************************************************/
static status_t 
    finish_bits_type (ncx_instance_t *instance,
                      tk_chain_t  *tkc,
                      ncx_module_t *mod,
                      typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        /* Got a token string so check the value */
        if (!xml_strcmp(instance, val, YANG_K_BIT)) {
            bitdone = TRUE;
            res = consume_yang_bit(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        } else {
            retres = ERR_NCX_WRONG_TKVAL;
            ncx_mod_exp_err(instance, tkc, mod, retres, expstr);
...
*********************************************************************/
static status_t 
    consume_yang_enum (ncx_instance_t *instance,
                       tk_chain_t *tkc,
                       ncx_module_t *mod,
                       typ_def_t *typdef)
{
    ...
    if (str) {
        if (!*str) {
            ...
            m__free(instance, str);
            str = NULL;
        } else if (xml_isspace(str[xml_strlen(instance, str)-1])) {
            res = ERR_NCX_INVALID_VALUE;
            log_error(instance,
        ...
        enu = typ_new_enum(instance, NCX_EL_NONE);
    }
    ...
    while (!done) {
        ...
        if (!xml_strcmp(instance, val, YANG_K_DESCRIPTION)) {
            ...
            res = yang_consume_status(instance, 
                                      ...
                                      &stat,
                                      &enu->appinfoQ);
        } else if (!xml_strcmp(instance, val, YANG_K_VALUE)) {
            res = yang_consume_int32(instance,
                                     tkc,
                                     mod,
                                     &enu->val, 
                                     &valdone,
...
*********************************************************************/
static status_t 
    finish_enum_type (ncx_instance_t *instance,
                      tk_chain_t  *tkc,
                      ncx_module_t *mod,
                      typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        /* Got a token string so check the value */
        if (!xml_strcmp(instance, val, YANG_K_ENUM)) {
            enumdone = TRUE;
            res = consume_yang_enum(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        } else {
            retres = ERR_NCX_WRONG_TKVAL;
            ncx_mod_exp_err(instance, tkc, mod, retres, expstr);
    ...
    if (!enumdone) {
        retres = ERR_NCX_DATA_MISSING;
        ncx_mod_missing_err(instance, tkc, mod, "enumeration", "enum");
    }
...
*********************************************************************/
static status_t 
    finish_union_type (ncx_instance_t *instance,
                       yang_pcb_t *pcb,
                       tk_chain_t  *tkc,
                       ncx_module_t *mod,
                       typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        if (!xml_strcmp(instance, val, YANG_K_TYPE)) {
            typedone = TRUE;
            un = typ_new_unionnode(instance, NULL);
            if (!un) {
                res = ERR_INTERNAL_MEM;
                ncx_print_errormsg(instance, tkc, mod, res);
                return res;
            } else {
                un->typdef = typ_new_typdef(instance);
                if (!un->typdef) {
                    res = ERR_INTERNAL_MEM;
                    ncx_print_errormsg(instance, tkc, mod, res);
            ...
            if (res == NO_ERR) {
                dlq_enque(instance, un, &typdef->def.simple.unionQ);
            } else {
                typ_free_unionnode(instance, un);
            }
        } else {
    ...
    if (!typedone) {
        retres = ERR_NCX_DATA_MISSING;
        ncx_mod_missing_err(instance, tkc, mod, "union", "type");
    }
...
*********************************************************************/
static status_t 
    finish_leafref_type (ncx_instance_t *instance,
                        tk_chain_t  *tkc,
                        ncx_module_t *mod,
                        typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        /* Got a token string so check the value */
        if (!xml_strcmp(instance, val, YANG_K_PATH)) {
            if (pathdone) {
                retres = ERR_NCX_ENTRY_EXISTS;
            ...
            if (TK_CUR_STR(tkc)) {
                ...
                if (!pathdone) {
                     ...
                     * context node
                     */
                    sim->xleafref = xpath_new_pcb(instance, TK_CUR_VAL(tkc), NULL);
                    if (!sim->xleafref) {
                        res = ERR_INTERNAL_MEM;
                        ncx_print_errormsg(instance, tkc, mod, res);
                        return res;
                ...
                res = yang_consume_semiapp(instance, tkc, mod, &typdef->appinfoQ);
                CHK_EXIT(res, retres);
            }
...
*********************************************************************/
static status_t 
    finish_instanceid_type (ncx_instance_t *instance,
                            tk_chain_t  *tkc,
                            ncx_module_t *mod,
                            typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        /* Got a token string so check the value */
        if (!xml_strcmp(instance, val, YANG_K_REQUIRE_INSTANCE)) {
...
*********************************************************************/
static status_t 
    finish_idref_type (ncx_instance_t *instance,
                       tk_chain_t  *tkc,
                       ncx_module_t *mod,
                       typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        /* Got a token string so check the value */
        if (!xml_strcmp(instance, val, YANG_K_BASE)) {
            sim->idref.modname = ncx_get_modname(mod);
            res = yang_consume_pid(instance, 
                                   tkc, 
                                   mod,
...
*********************************************************************/
static status_t 
    resolve_minmax (ncx_instance_t *instance,
                    tk_chain_t *tkc,
                    ncx_module_t *mod,
                    typ_def_t *typdef,
                    boolean simple,
                    ncx_btype_t  rbtyp)
{
    ...
    if (!simple) {
        parentdef = typ_get_parent_typdef(instance, typdef);
        rangedef = typ_get_qual_typdef(instance, parentdef, NCX_SQUAL_RANGE);
        if (rangedef) {
            parentQ = typ_get_rangeQ_con(instance, rangedef);
            if (!parentQ || dlq_empty(instance, parentQ)) {
                return SET_ERROR(instance, ERR_INTERNAL_VAL);
            }
            pfirstrv = (typ_rangedef_t *)dlq_firstEntry(instance, parentQ);
            plastrv = (typ_rangedef_t *)dlq_lastEntry(instance, parentQ);
        }
    }

    for (rv = (typ_rangedef_t *)dlq_firstEntry(instance, rangeQ);
         rv != NULL && res==NO_ERR;
        ...
        if (rv->flags & TYP_FL_LBMIN) {
            if (pfirstrv) {
                if (pfirstrv->flags & TYP_FL_LBINF) {
                    ...
                    res = ncx_copy_num(instance, &pfirstrv->lb, &rv->lb, rbtyp);
                }
            } else {
                if (rbtyp==NCX_BT_FLOAT64) {
                    rv->flags |= TYP_FL_LBINF;
                } else {
                    ncx_set_num_min(instance, &rv->lb, rbtyp);
                }
            }
        } else if (rv->flags & TYP_FL_LBMAX) {
            if (plastrv) {
                if (plastrv->flags & TYP_FL_UBINF) {
                    ...
                    res = ncx_copy_num(instance, &plastrv->ub, &rv->lb, rbtyp);
                }
            } else {
                if (rbtyp==NCX_BT_FLOAT64) {
                    rv->flags |= TYP_FL_LBINF2;
                } else {
                    ncx_set_num_max(instance, &rv->lb, rbtyp);
                }
            }
        } else if (rv->lbstr) {
            res = ncx_decode_num(instance, rv->lbstr, rbtyp, &rv->lb);
        }
        ...
        if (rv->flags & TYP_FL_UBMAX) {
            if (plastrv) {
                if (plastrv->flags & TYP_FL_UBINF) {
                    ...
                    res = ncx_copy_num(instance, &plastrv->ub, &rv->ub, rbtyp);
                }
            } else {
                if (rbtyp==NCX_BT_FLOAT64) {
                    rv->flags |= TYP_FL_UBINF;
                } else {
                    ncx_set_num_max(instance, &rv->ub, rbtyp);
                }
            }
            ...
            if (pfirstrv) {
                if (pfirstrv->flags & TYP_FL_LBINF) {
                    ...
                    res = ncx_copy_num(instance, &pfirstrv->lb, &rv->ub, rbtyp);
                }
            } else {
                if (rbtyp==NCX_BT_FLOAT64) {
                    rv->flags |= TYP_FL_UBINF2;
                } else {
                    ncx_set_num_min(instance, &rv->ub, rbtyp);
                }
            }
        } else if (rv->ubstr) {
            res = ncx_decode_num(instance, rv->ubstr, rbtyp, &rv->ub);
        }
    }
    ...
    if (res != NO_ERR) {
         ...
         */
        rv = (typ_rangedef_t *)dlq_firstEntry(instance, rangeQ);
        if (rv && ncx_is_min(instance, &rv->lb, rbtyp)) {
            rv->flags |= TYP_FL_LBMIN;
        }
        rv = (typ_rangedef_t *)dlq_lastEntry(instance, rangeQ);
        if (rv && ncx_is_max(instance, &rv->ub, rbtyp)) {
            rv->flags |= TYP_FL_UBMAX;
        }
    }
...
*********************************************************************/
static status_t 
    finish_yang_range (ncx_instance_t *instance,
                       tk_chain_t  *tkc,
                       ncx_module_t *mod,
                       typ_def_t *typdef,
                       ncx_btype_t rbtyp)
{
    ...
    case NCX_CL_NAMED:
        parentdef = typ_get_parent_typdef(instance, typdef);
        if (parentdef) {
            res = finish_yang_range(instance, 
                                    tkc, 
                                    mod,
                                    parentdef,
                                    rbtyp);
        ...
        if (res == NO_ERR) {
            res = resolve_minmax(instance, tkc, mod, typdef, FALSE, rbtyp);
            if (res == NO_ERR) {
                rangeQ = typ_get_rangeQ_con(instance, typdef);
                typ_normalize_rangeQ(instance, rangeQ, rbtyp);
    ...
    case NCX_CL_SIMPLE:
         ...
         */
        res = resolve_minmax(instance, tkc, mod, typdef, TRUE, rbtyp);
        if (res == NO_ERR) {
            rangeQ = typ_get_rangeQ_con(instance, typdef);
            typ_normalize_rangeQ(instance, rangeQ, rbtyp);
        } else if (res == ERR_NCX_SKIPPED) {
...
*********************************************************************/
static status_t
    rv_fits_in_rangedef (ncx_instance_t *instance,
                         ncx_btype_t  btyp,
                         ...
                         const typ_rangedef_t *checkrv)
{
    ...
    if (!(checkrv->flags & TYP_FL_LBINF)) {
        cmp = ncx_compare_nums(instance, &testrv->lb, &checkrv->lb, btyp);
        if (cmp >= 0) {
            lbok = TRUE;
        }
    } else {
    ...
    if (!(checkrv->flags & TYP_FL_UBINF)) {
        cmp = ncx_compare_nums(instance, &testrv->lb, &checkrv->ub, btyp);
        if (cmp <= 0) {
            lbok2 = TRUE;
        }
    } else {
    ...
    /* check upper bound */
    if (!(checkrv->flags & TYP_FL_LBINF)) {
        cmp = ncx_compare_nums(instance, &testrv->ub, &checkrv->lb, btyp);
        if (cmp >= 0) {
            ubok = TRUE;
        }
    } else {
    ...
    if (!(checkrv->flags & TYP_FL_UBINF)) {
        cmp = ncx_compare_nums(instance, &testrv->ub, &checkrv->ub, btyp);
        if (cmp <= 0) {
            ubok2 = TRUE;
        }
    } else {
...
*********************************************************************/
static status_t 
    validate_range_chain (ncx_instance_t *instance,
                          tk_chain_t  *tkc,
                          ncx_module_t *mod,
                          typ_def_t *intypdef,
                          const xmlChar *typname)
{
    ...
    typdef = typ_get_qual_typdef(instance, intypdef, NCX_SQUAL_RANGE);
    if (!typdef) {
        return NO_ERR;
    }
    ...
    res = NO_ERR;
    errdone = FALSE;
    rbtyp = typ_get_range_type(typ_get_basetype(instance, typdef));
    if (!typname) {
    ...
    while (typdef && res==NO_ERR) {
        ...
        for (rv = (typ_rangedef_t *)dlq_firstEntry(instance, rangeQ);
            ...
            retval = ncx_compare_nums(instance, &rv->lb, &rv->ub, rbtyp);
            if (retval == 1) {
                res = ERR_NCX_INVALID_RANGE;
                log_error(instance,
            ...
            if (rv->flags & TYP_FL_UBINF) {
                ...
                retval = ncx_compare_nums(instance, &rv->lb, curmax, rbtyp);
                if (retval != 1) {
                    res = ERR_NCX_OVERLAP_RANGE;
                } else {
                    curmax = &rv->ub;
        ...
        if (res == NO_ERR) {
            ...
            if (typdef) {
                typdef = typ_get_qual_typdef(instance, typdef, NCX_SQUAL_RANGE);
            }
        } else {
            tkc->curerr = &typdef->tkerr;
        }
    ...
    while (typdef && res==NO_ERR) {
        ...
        for (rv = (typ_rangedef_t *)dlq_firstEntry(instance, rangeQ);
            ...
            parentdef = typ_get_parent_typdef(instance, typdef);
            if (parentdef) {
                parentdef = typ_get_qual_typdef(instance, parentdef, NCX_SQUAL_RANGE);

                while (parentdef && res==NO_ERR) {
                    prangeQ = typ_get_rangeQ_con(instance, parentdef);

                    /* rv rangedef just has to fit within one of the checkrv 
                     * rangedefs to pass the test */
                    done = FALSE;
                    for (checkrv = (typ_rangedef_t *)dlq_firstEntry(instance, prangeQ);
                         checkrv != NULL && !done;
                         checkrv = (typ_rangedef_t *)
                             dlq_nextEntry(instance, checkrv)) {
                        res2 = rv_fits_in_rangedef(instance, rbtyp, rv, checkrv);
                        if (res2 == NO_ERR) {
                            done = TRUE;
                        }
                    }
                    ...
                    if (!done) {
                        ...
                        parentdef = typ_get_parent_typdef(instance, parentdef);
                        if (parentdef) {
                            parentdef = 
...
*********************************************************************/
static status_t 
    check_defval (ncx_instance_t *instance,
                  tk_chain_t *tkc,
                  ncx_module_t *mod,
                  typ_def_t *typdef,
                  obj_template_t *obj,
    ...
    if (defval != NULL) {
        *hasdefval = TRUE;        
        if (btyp == NCX_BT_IDREF) {
            res = val_parse_idref(instance, mod, defval, NULL, NULL, NULL);
        } else {
            res = val_simval_ok(instance, typdef, defval);
        }
        if (res != NO_ERR) {
            if (!badvalok) {
                if (obj) {
                    log_error(instance,
                              ...
                              "default value (%s)",
                              (name) ? "Type" : 
                              (const char *)obj_get_typestr(instance, obj),
                              (name) ? name : obj_get_name(instance, obj), 
                              defval);
                } else {
                    log_error(instance,
                              "\nError: %s '%s' has invalid "
        ...
        typdefval = typ_get_defval(instance, typdef->def.named.typ);
        if (typdefval) {
            *hasdefval = TRUE;
            if (btyp == NCX_BT_IDREF) {
                ...
                res = val_simval_ok(instance, typdef, typdefval);
            }
            if (res != NO_ERR) {
                if (!badvalok) {
                    if (obj) {
...
*********************************************************************/
static status_t 
    resolve_union_type (ncx_instance_t *instance,
                        yang_pcb_t *pcb,
                        tk_chain_t *tkc,
                        ncx_module_t *mod,
                        dlq_hdr_t  *unionQ,
                        ...
                        const xmlChar *name)
{
    ...
    for (un = (typ_unionnode_t *)dlq_firstEntry(instance, unionQ);
         un != NULL && retres == NO_ERR;
         un = (typ_unionnode_t *)dlq_nextEntry(instance, un)) {

        res = resolve_type(instance, pcb, tkc,  mod, un->typdef, NULL, 
                           NULL, parent, grp);
        CHK_EXIT(res, retres);

        btyp = typ_get_basetype(instance, un->typdef);

        if (btyp != NCX_BT_NONE && !typ_ok_for_union(instance, btyp)) {
        ...
        if ((retres == NO_ERR) && 
            !defvaldone && defval &&
            (btyp != NCX_BT_NONE) &&
            typ_ok(instance, un->typdef)) {

            res = check_defval(instance, tkc, mod, un->typdef, parent, btyp, 
                               TRUE, defval, name);
            if (res == NO_ERR) {
                defvaldone = TRUE;
            }
    ...
    if (retres == NO_ERR && defval != NULL && !defvaldone) {
        ...
        if (parent != NULL) {
            log_error(instance,
                      ...
                      "default value (%s)",
                      (name) ? "type" : 
                      (const char *)obj_get_typestr(instance, parent),
                      (name) ? name : obj_get_name(instance, parent), 
                      defval);
        } else {
            log_error(instance,
...
*********************************************************************/
static status_t 
    resolve_union_type_final (ncx_instance_t *instance,
                              tk_chain_t *tkc,
                              ncx_module_t *mod,
                              dlq_hdr_t *unionQ,
                              obj_template_t *parent,
                              ...
                              const xmlChar *name)
{
    ...
    for (un = (typ_unionnode_t *)dlq_firstEntry(instance, unionQ);
        ...
        if (!typ_ok(instance, un->typdef)) {
            return NO_ERR;
        }

        btyp = typ_get_basetype(instance, un->typdef);
        if (btyp == NCX_BT_LEAFREF || btyp == NCX_BT_UNION ||
            typ_is_xpath_string(instance, un->typdef)) {
            needtest = TRUE;
        }
    }
    ...
    for (un = (typ_unionnode_t *)dlq_firstEntry(instance, unionQ);
        ...
        if (defval && !defvaldone && (btyp != NCX_BT_NONE)) {
            res = check_defval(instance, tkc, mod, un->typdef, parent, btyp, TRUE, 
                               defval, name);
            if (res == NO_ERR) {
                defvaldone = TRUE;
            }
...
*********************************************************************/
static status_t 
    resolve_type (ncx_instance_t *instance,
                  yang_pcb_t *pcb,
                  tk_chain_t *tkc,
                  ncx_module_t  *mod,
                  typ_def_t *typdef,
                  ...
                  grp_template_t *grp)
{
    ...
    /* check the appinfoQ */
    res = ncx_resolve_appinfoQ(instance, pcb, tkc, mod, &typdef->appinfoQ);
    if (NEED_EXIT(res)) {
        return res;
    } else {
        res = NO_ERR;
    ...
    if (res == NO_ERR && btyp != NCX_BT_UNION) {
        res = obj_set_named_type(instance, tkc, mod, name, typdef, obj, grp);
    }

    /* type name loop check */
    if (res == NO_ERR && btyp != NCX_BT_UNION) {
        res = loop_test(instance, typdef);
    }

    /* If no loops then make sure base type is correct */
    if (res == NO_ERR && btyp != NCX_BT_UNION) {
        res = restriction_test(instance, tkc, mod, typdef);
        if (res != NO_ERR) {
            errdone = TRUE;
        }
    }
     ...
     */
    if (btyp != NCX_BT_UNION) {
        res = finish_yang_range(instance, tkc, mod, typdef,
                                typ_get_range_type(typ_get_basetype(instance, typdef)));
    }
    ...
    if (res == NO_ERR && btyp != NCX_BT_UNION) {
        ...
        res = validate_range_chain(instance, tkc, mod, typdef, errname);
    }

    /* check default value if any defined */
    if ((res == NO_ERR) &&
        (btyp != NCX_BT_NONE) && 
        (btyp != NCX_BT_UNION) && typ_ok(instance, typdef)) {

        if (btyp == NCX_BT_LEAFREF || typ_is_xpath_string(instance, typdef)) {
            if (LOGDEBUG4) {
                log_debug4(instance, 
            ...
            if (defval) {
                res = check_defval(instance, tkc, mod, typdef, obj, btyp, FALSE, 
                                   defval, name);
            }
        }
    }
        ...
        case NCX_BT_BITS:
            ...
            if (typdef->tclass == NCX_CL_SIMPLE) {
                for (enu = (typ_enum_t *)
                     ...
                     enu != NO_ERR;
                     enu = (typ_enum_t *)dlq_nextEntry(instance, enu)) {

                    res = ncx_resolve_appinfoQ(instance, pcb, tkc, mod, &enu->appinfoQ);
                    CHK_EXIT(res, retres);
                }
                res = NO_ERR;
            }
            break;
        case NCX_BT_IDREF:
            idref = typ_get_idref(instance, typdef);
            if (idref) {
                testidentity = NULL;
                if (idref->baseprefix &&
                    xml_strcmp(instance, idref->baseprefix, mod->prefix)) {

                    /* find the identity in another module */
                    res = yang_find_imp_identity(instance, pcb, tkc, mod, 
                                                 idref->baseprefix,
                                                 idref->basename, 
                                                 &typdef->tkerr,
                                                 &testidentity);
                } else {
                    testidentity = ncx_find_identity(instance, mod, idref->basename, 
                                                     FALSE);
                    if (!testidentity) {
    ...
    if (res == NO_ERR &&
        ...
        btyp == NCX_BT_UNION) {
        testdef = typ_get_base_typdef(instance, typdef);
        res = resolve_union_type(instance, pcb, tkc, mod, &testdef->def.simple.unionQ,
                                 obj, grp, defval, name);
    }
    ...
    if (name != NULL) {
        ...
        if (obj && obj->grp && obj->grp->parentgrp) {
            ...
            while (nextgrp) {
                errtyp = ncx_find_type_que(instance, &nextgrp->typedefQ, name);
                if (errtyp) {
                    nextgrp = NULL;
                } else {
        ...
        if (!errtyp && obj && obj->parent && !obj_is_root(obj->parent)) {
            errtyp = obj_find_type(instance, obj->parent, name);
        }
         ...
         */
        if (!errtyp && obj ) {
            errtyp = ncx_find_type(instance, mod, name, TRUE);
        }
...
*********************************************************************/
static status_t 
    resolve_type_final (ncx_instance_t *instance,
                        tk_chain_t *tkc,
                        ncx_module_t  *mod,
                        obj_template_t *obj,
                        typ_def_t *typdef,
                        const xmlChar *defval)
{
    ...
    if (!defval || !typ_ok(instance, typdef) || typdef->tclass == NCX_CL_BASE ) {
    ...
    if (btyp == NCX_BT_LEAFREF || btyp == NCX_BT_INSTANCE_ID) {
        ...
        leafobj = NULL;
        if (!obj) {
            obj = ncx_get_gen_root(instance);
        }
        
        res = xpath_yang_validate_path(instance, mod, obj, pcb, FALSE, &leafobj);
        if (res == NO_ERR && leafobj != NULL) {
            typ_set_xref_typdef(instance, typdef, obj_get_typdef(leafobj));
    ...
    if (btyp == NCX_BT_UNION) {
        if (defval) {
            res = check_defval(instance, tkc, mod, typdef, obj, btyp, FALSE, defval, 
                               typname);
        }
    } else if (typdef->tclass == NCX_CL_SIMPLE) {
        /* special check for union typdefs, errors printed by called fn */
        res = resolve_union_type_final(instance, tkc, mod, &typdef->def.simple.unionQ,
                                       obj, defval,
                                       (obj) ? obj_get_name(instance, obj) : NULL);
...
*********************************************************************/
static status_t 
    resolve_typedef (ncx_instance_t *instance,
                     yang_pcb_t *pcb,
                     tk_chain_t *tkc,
                     ncx_module_t  *mod,
                     typ_template_t *typ,
                     ...
                     grp_template_t *grp)
{
    ...
    /* check the appinfoQ */
    typ->res = res = ncx_resolve_appinfoQ(instance, pcb, tkc, mod, &typ->appinfoQ);
    CHK_EXIT(res, retres);

    typ->res = res = resolve_type(instance, pcb, tkc, mod, &typ->typdef, typ->name,
                                  typ->defval, obj, grp);
    CHK_EXIT(res, retres);
...
*********************************************************************/
static status_t 
    resolve_typedef_final (ncx_instance_t *instance,
                           tk_chain_t *tkc,
                           ncx_module_t  *mod,
                           typ_template_t *typ)
{
    ...
    typ->res = res = resolve_type_final(instance, tkc, mod, NULL,  &typ->typdef, 
                                        typ->defval);
    return res;
...
*********************************************************************/
static status_t 
    consume_type (ncx_instance_t *instance,
                  yang_pcb_t *pcb,
                  tk_chain_t *tkc,
                  ncx_module_t  *mod,
                  typ_def_t *intypdef,
                  boolean metamode)
{
    ...
    if (intypdef->prefix && intypdef->typenamestr &&
        xml_strcmp(instance, intypdef->prefix, mod->prefix)) {
        /* real import - not the same as this module's prefix */
        res = yang_find_imp_typedef(instance,
                                    pcb,
                                    tkc, 
                                    mod,
                                    intypdef->prefix,
        ...
        if (res != NO_ERR) {
            ...
            typ_init_named(instance, intypdef);
        } else {
            /* found named type OK, setup typdef */
            typ_init_named(instance, intypdef);
            btyp = typ_get_basetype(instance, &imptyp->typdef);
            typ_set_named_typdef(instance, intypdef, imptyp);
        }
         ...
         */
        btyp = tk_get_yang_btype_id(instance,
                                    intypdef->typenamestr,
                                    xml_strlen(instance, intypdef->typenamestr));
        if (btyp != NCX_BT_NONE) {
            /* base type is builtin type */
            typ_init_simple(instance, intypdef, btyp);
        } else {
    ...
    if (TK_CUR_TYP(tkc) == TK_TT_SEMICOL || metamode) {
        ...
        case NCX_BT_LEAFREF:
            ...
            break;
        case NCX_BT_NONE:
            if (metamode && typ_get_basetype(instance, intypdef) == NCX_BT_NONE) {
                retres = obj_set_named_type(instance, 
                                            tkc, 
                                            mod, 
                                            NULL,
                                            intypdef, 
    ...
    if (imptyp || btyp == NCX_BT_NONE) {
        res = typ_set_new_named(instance, intypdef, btyp);
        if (res == NO_ERR) {
            typdef = typ_get_new_named(instance, intypdef);
            derived = TRUE;
    ...
    case NCX_BT_NONE:
        ...
        res = finish_unknown_type(instance, tkc, mod, typdef);
        CHK_EXIT(res, retres);
        break;
    case NCX_BT_ANY:
        extonly = TRUE;
    ...
    case NCX_BT_BITS:
        if (derived) {
            ...
            res = finish_bits_type(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        }
        break;
    case NCX_BT_ENUM:
        if (derived) {
            ...
            res = finish_enum_type(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        }
        break;
    case NCX_BT_EMPTY:
    ...
    case NCX_BT_FLOAT64:
        res = finish_number_type(instance, tkc, mod, btyp, typdef);
        CHK_EXIT(res, retres);
        break;
    case NCX_BT_STRING:
    case NCX_BT_BINARY:
        res = finish_string_type(instance, tkc, mod, typdef, btyp);
        CHK_EXIT(res, retres);
        break;
    case NCX_BT_UNION:
        if (derived) {
            ...
            res = finish_union_type(instance,
                                    pcb,
                                    tkc, 
                                    mod, 
                                    typdef);
    ...
    case NCX_BT_LEAFREF:
        if (derived) {
            ...
            res = finish_leafref_type(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        }
        break;
    case NCX_BT_IDREF:
        if (derived) {
            ...
            res = finish_idref_type(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        }
        break;
    case NCX_BT_INSTANCE_ID:
        if (derived) {
            ...
            res = finish_instanceid_type(instance, tkc, mod, typdef);
            CHK_EXIT(res, retres);
        }
        break;
    default:
...
*********************************************************************/
status_t 
    yang_typ_consume_type (ncx_instance_t *instance,
                           yang_pcb_t *pcb,
                           tk_chain_t *tkc,
                           ncx_module_t  *mod,
                           typ_def_t *intypdef)
...
*   status of the operation
*********************************************************************/
status_t yang_typ_consume_metadata_type(ncx_instance_t *instance,
                                          yang_pcb_t *pcb,
                                         tk_chain_t *tkc,
                                         ncx_module_t  *mod,
                                         typ_def_t *intypdef )
{
    ...
    return consume_type(instance, pcb, tkc, mod, intypdef, TRUE);
...
*********************************************************************/
status_t 
    yang_typ_consume_typedef (ncx_instance_t *instance,
                              yang_pcb_t *pcb,
                              tk_chain_t *tkc,
                              ncx_module_t  *mod,
                              dlq_hdr_t *que)
{
    ...
    res = yang_consume_id_string(instance, tkc, mod, &typ->name);
    ...
    while (!done) {
        ...
        case TK_TT_MSTRING:
             ...
             * typ_template_t
             */
            res = ncx_consume_appinfo(instance, tkc, mod, &typ->typdef.appinfoQ);
            if (res != NO_ERR) {
                retres = res;
                if (NEED_EXIT(res)) {
                    typ_free_template(instance, typ);
    ...
    if (typ->name && ncx_valid_name2(instance, typ->name)) {
        testtyp = ncx_find_type_que(instance, que, typ->name);
        if (testtyp == NULL) {
            testtyp = ncx_find_type(instance, mod, typ->name, TRUE);
        }
        if (testtyp) {
...
*********************************************************************/
status_t 
    yang_typ_resolve_typedefs (ncx_instance_t *instance,
                               yang_pcb_t *pcb,
                               tk_chain_t *tkc,
                               ncx_module_t  *mod,
                               dlq_hdr_t *typeQ,
                               obj_template_t *parent)
{
    ...
    /*
     * RIFT: The original loop here fails on 2013-07-15 ietf-inet-types.yang
     * because it tries to resolve a union of following typedefs that have
     * restrictions, but before those typedefs have been resolved by this
     * loop.  The change is to order the processing: first simple non-union
     * types, then unions and typedefs.
     */
    ...
    for (typ = (typ_template_t *)dlq_firstEntry(instance, typeQ);
        ...
        if (typ->typdef.tclass == NCX_CL_SIMPLE && typ->typdef.def.simple.btyp != NCX_BT_UNION) {
            res = resolve_typedef(instance,
                                  pcb,
                                  tkc, 
                                  mod, 
                                  typ, 
...
*********************************************************************/
status_t 
    yang_typ_resolve_typedefs_final (ncx_instance_t *instance,
                                     tk_chain_t *tkc,
                                     ncx_module_t *mod,
                                     dlq_hdr_t *typeQ)
{
    ...
    for (typ = (typ_template_t *)dlq_firstEntry(instance, typeQ);
        ...
        res = resolve_typedef_final(instance, tkc, mod, typ);
        CHK_EXIT(res, retres);
    }
...
*********************************************************************/
status_t 
    yang_typ_resolve_typedefs_grp (ncx_instance_t *instance,
                                   yang_pcb_t *pcb,
                                   tk_chain_t *tkc,
                                   ncx_module_t  *mod,
                                   dlq_hdr_t *typeQ,
                                   ...
                                   grp_template_t *grp)
{
    ...
    for (typ = (typ_template_t *)dlq_firstEntry(instance, typeQ);
        ...
        res = resolve_typedef(instance, pcb, tkc, mod, typ, parent, grp);
        CHK_EXIT(res, retres);
    }
...
*********************************************************************/
status_t 
    yang_typ_resolve_type (ncx_instance_t *instance,
                           yang_pcb_t *pcb,
                           tk_chain_t *tkc,
                           ncx_module_t  *mod,
                           typ_def_t *typdef,
                           ...
                           obj_template_t *obj)
{
    ...
    res = resolve_type(instance, pcb, tkc, mod, typdef, NULL, defval, obj, NULL);
    return res;
...
*********************************************************************/
status_t 
    yang_typ_resolve_type_final (ncx_instance_t *instance,
                                 tk_chain_t *tkc,
                                 ncx_module_t *mod,
                                 typ_def_t *typdef,
                                 const xmlChar *defval,
                                 obj_template_t *obj)
{
    ...
    res = resolve_type_final(instance, tkc, mod, obj, typdef, defval);
    return res;
...
*********************************************************************/
status_t 
    yang_typ_rangenum_ok (ncx_instance_t *instance,
                          typ_def_t *typdef,
                          const ncx_num_t *num)
{
    typ_def_t   *rdef;
    ...
    rdef = typ_get_qual_typdef(instance, typdef, NCX_SQUAL_RANGE);
    while (rdef && res==NO_ERR) {
        res = val_range_ok(instance, rdef, rbtyp, num);
        if (res != NO_ERR) {
            continue;
        }
        rdef = typ_get_parent_typdef(instance, rdef);
        if (rdef) {
            rdef = typ_get_qual_typdef(instance, rdef, NCX_SQUAL_RANGE);
        }
    }
