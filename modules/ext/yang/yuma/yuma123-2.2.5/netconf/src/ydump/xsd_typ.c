...
*********************************************************************/
static status_t
    add_err_annotation (ncx_instance_t *instance,
                        ...
                        val_value_t  *val)
{
    ...
    if (errmsg || errtag || descr || ref) {
        anval = xml_val_new_struct(instance, XSD_ANNOTATION, xmlns_xs_id(instance));
        if (!anval) {
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, anval, val);
        }

        if (descr) {
            res = xsd_do_documentation(instance, descr, anval);
            if (res != NO_ERR) {
                return res;
            }
        }
        ...
        if (errmsg || errtag || ref) {
            chval = xsd_make_err_appinfo(instance, ref, errmsg, errtag);
            if (!chval) {
                return ERR_INTERNAL_MEM;
            } else {
                val_add_child(instance, chval, anval);
            }
        }
    }
    return NO_ERR;
...
*********************************************************************/
static val_value_t *
    new_restriction (ncx_instance_t *instance,
...

{
    ...
    if (hasnodes) {
        val = xml_val_new_struct(instance, XSD_RESTRICTION, xsd_id);
    } else {
        val = xml_val_new_flag(instance, XSD_RESTRICTION, xsd_id);
    }
    ...
    if (basename) {
        if (mod->nsid == base_nsid) {
            ...
            res = xml_val_add_cattr(instance, XSD_BASE, 0, basename, val);
        } else {
            /* not the target namespace, so need a QName instead of Name */
            str = xml_val_make_qname(instance, base_nsid, basename);
            if (str) {
                res = xml_val_add_attr(instance, XSD_BASE, 0, str, val);
                if (res != NO_ERR) {
...
*********************************************************************/
static status_t
    add_pattern (ncx_instance_t *instance,
                 ...
                 val_value_t *val)
{
    ...
    if (errinfo_set || pat->pat_errinfo.descr || 
        ...
        patval = xml_val_new_struct(instance, NCX_EL_PATTERN, xsd_id);
        if (!patval) {
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, patval, val);
            res = add_err_annotation(instance, 
                                     pat->pat_errinfo.error_message, 
        ...
        patval = xml_val_new_flag(instance, NCX_EL_PATTERN, xsd_id);
        if (!patval) {
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, patval, val);
    ...
    res = xml_val_add_cattr(instance, NCX_EL_VALUE, 0, pat->pat_str, patval);
    if (res != NO_ERR) {
        return res;
    }
...
*********************************************************************/
static status_t
    add_patterns (ncx_instance_t *instance,
                  const ncx_module_t *mod,
                  ...
                  val_value_t *val)
{
    ...
    pat = NULL;
    xsd_id = xmlns_xs_id(instance);
    patcnt = typ_get_pattern_count(instance, typdef);

    if (patcnt == 0) {
        return NO_ERR;
    } else if (patcnt == 1) {
        return add_pattern(instance, typ_get_first_cpattern(instance, typdef), val);
    }
    ...
    for (patnum = 1; patnum < patcnt; patnum++) {
        ...
        simval = xml_val_new_struct(instance, XSD_SIM_TYP, xsd_id);
        if (!simval) {
            if (curtop) {
                val_free_value(instance, curtop);
            }
            return ERR_INTERNAL_MEM;
        }

        if (patnum == 1) {
            restval = new_restriction(instance, mod, base_nsid, basename, TRUE);
        } else {
            restval = new_restriction(instance, mod, 0, NULL, TRUE);
        }
        if (!restval) {
            if (curtop) {
                val_free_value(instance, curtop);
            ...
            val_add_child(instance, restval, simval);
        }

        if (!curtop) {
            curtop = simval;
        } else {
            val_add_child(instance, curtop, restval);
            curtop = simval;
        }

        res = add_pattern(instance, pat, restval);
        if (res != NO_ERR) {
            val_free_value(instance, curtop);
            return res;
        }
    ...
    val_add_child(instance, curtop, val);

    /* add the last pattern as a sibling of the new simpleType tree */
    pat = typ_get_next_cpattern(instance, pat);
    res = add_pattern(instance, pat, val);    
    return res;
...
*********************************************************************/
static status_t
    add_enum_restriction (ncx_instance_t *instance,
                          const typ_enum_t *enu,
                          val_value_t *val)
{
    ...
    enuval = xml_val_new_struct(instance, XSD_ENUMERATION, xsd_id);
    if (!enuval) {
        return ERR_INTERNAL_MEM;
    } else {
        val_add_child(instance, enuval, val);  /* add early */
    }

    /* add value attribute */
    res = xml_val_add_cattr(instance, NCX_EL_VALUE, 0, enu->name, enuval);
    if (res != NO_ERR) {
        return res;
    }
    ...
    if (!annot) {
        ...
        val_add_child(instance, annot, enuval);  /* add early */
    }

    /* add documentation child element */
    if (enu->descr) {
        res = xsd_do_documentation(instance, enu->descr, annot);
        if (res != NO_ERR) {
            return res;
        }
    }
    ...
    if (enu->ref) {
        res = xsd_do_reference(instance, enu->ref, annot);
        if (res != NO_ERR) {
            return res;
        }
    }
    ...
    if (enu->flags & TYP_FL_ISBITS) {
        appval = xsd_make_bit_appinfo(instance, 
                                      enu->pos, 
                                      &enu->appinfoQ,
                                      enu->status);
    } else {
        appval = xsd_make_enum_appinfo(instance, 
                                       enu->val, 
                                       &enu->appinfoQ,
                                       enu->status);
    }
...
*********************************************************************/
static val_value_t *
    make_enum (ncx_instance_t *instance,
               const ncx_module_t *mod,
               const typ_def_t *typdef)
{
    val_value_t      *chval;
    ...
    chval = new_restriction(instance, mod, xmlns_xs_id(instance), NCX_EL_STRING, TRUE);
    if (!chval) {
        return NULL;
    }

    /* add all the enumeration strings */
    for (typ_enu = typ_first_con_enumdef(instance, typdef);
         typ_enu != NULL;
         typ_enu = (const typ_enum_t *)dlq_nextEntry(instance, typ_enu)) {
        res = add_enum_restriction(instance, typ_enu, chval);
        if (res != NO_ERR) {
...
*********************************************************************/
static status_t
    finish_range (ncx_instance_t *instance,
                  const typ_def_t *typdef,
                  ...
                  val_value_t *val)
{
    ...
    case NCX_BT_DECIMAL64:
        if (rdef->flags & TYP_FL_LBMIN) {
            ...
            res = ncx_sprintf_num(instance, numbuff, &rdef->lb, rdef->btyp, &len);
            if (res != NO_ERR) {
                return res;
            }
        }
    ...
    if (!skipit) {
        if (errinfo_set) {
            if (srange) {
                chval = xml_val_new_struct(instance, XSD_MIN_LEN, xsd_id);
            } else {
                chval = xml_val_new_struct(instance, XSD_MIN_INCL, xsd_id);
            }
        } else {
            if (srange) {
                chval = xml_val_new_flag(instance, XSD_MIN_LEN, xsd_id);
            } else {
                chval = xml_val_new_flag(instance, XSD_MIN_INCL, xsd_id);
            }
        }
        if (!chval) {
            return ERR_INTERNAL_MEM;
        ...
        res = xml_val_add_cattr(instance, NCX_EL_VALUE, 0, numbuff, chval);
        if (res != NO_ERR) {
            return res;
        }
        ...
        if (errinfo_set) {
            res = add_err_annotation(instance, 
                                     range->range_errinfo.error_message, 
    ...
    case NCX_BT_DECIMAL64:
        if (rdef->flags & TYP_FL_UBMAX) {
            ...
            res = ncx_sprintf_num(instance, numbuff, &rdef->ub, rdef->btyp, &len);
            if (res != NO_ERR) {
                return res;
            }
        }
    ...
    if (!skipit) {
        if (errinfo_set) {
            if (srange) {
                chval = xml_val_new_struct(instance, XSD_MAX_LEN, xsd_id);
            } else {
                chval = xml_val_new_struct(instance, XSD_MAX_INCL, xsd_id);
            }
        } else {
            if (srange) {
                chval = xml_val_new_flag(instance, XSD_MAX_LEN, xsd_id);
            } else {
                chval = xml_val_new_flag(instance, XSD_MAX_INCL, xsd_id);
            }
        }
        if (!chval) {
            return ERR_INTERNAL_MEM;
...
*********************************************************************/
static val_value_t *
    new_range_union (ncx_instance_t *instance,
                     const ncx_module_t  *mod,
                     const typ_range_t *range,
                     ...
                     const xmlChar *basename)
{
    ...
    pattern_cnt = typ_get_pattern_count(instance, typdef);

    val = xml_val_new_struct(instance, XSD_UNION, xsd_id);
    if (!val) {
        return NULL;
    }
     ...
     * for each one
     */
    for (rv = (const typ_rangedef_t *)dlq_firstEntry(instance, rangeQ);
         rv != NULL;
         rv = (const typ_rangedef_t *)dlq_nextEntry(instance, rv)) {

        chval = xml_val_new_struct(instance, XSD_SIM_TYP, xsd_id);
        if (!chval) {
            val_free_value(instance, val);
            return NULL;
        } else {
            val_add_child(instance, chval, val);  /* add early */
        }

        rval = new_restriction(instance, mod, base_id, 
        ...
        if (!rval) {
            ...
            val_add_child(instance, rval, chval);  /* add early */
        }

        res = add_patterns(instance, mod, typdef, base_id, basename, rval);
        if (res != NO_ERR) {
            val_free_value(instance, val);
            return NULL;
        }

        res = finish_range(instance, typdef, range, rv, rval);
        if (res != NO_ERR) {
            val_free_value(instance, val);
            return NULL;
        }
...
*********************************************************************/
static status_t
    finish_list (ncx_instance_t *instance,
                 const ncx_module_t *mod,
                 ncx_btype_t  btyp,
                 typ_def_t *typdef,
                 val_value_t *val)
{
    ...
    if (btyp == NCX_BT_SLIST) {
        ...
        list_id = typ_get_nsid(instance, listtyp);
     ...
     * allowed
     */
    rtypdef = typ_get_cqual_typdef(instance, typdef, NCX_SQUAL_RANGE);
    if (rtypdef) {
        /* complex form with inline restriction */
        top = xml_val_new_struct(instance, XSD_LIST, xsd_id);
        if (!top) {
            return ERR_INTERNAL_MEM;
        }
    } else {
        /* simple form with itemType='QName' */
        top = xml_val_new_flag(instance, XSD_LIST, xsd_id);
        if (top) {
            /* construct the itemType="foo:bar" attribute */
            if (list_id != mod->nsid) {
                str = xml_val_make_qname(instance, list_id, typname);
            } else {
                str = xml_strdup(instance, typname);
            }
            if (!str) {
            ...
            res = xml_val_add_attr(instance, XSD_ITEMTYPE, 0, str, top);
            if (res != NO_ERR) {
                m__free(instance, str);
                val_free_value(instance, top);
                return res;
    ...
    if (rtypdef) {
        res = typ_get_rangebounds_con(instance, rtypdef, &range_btyp,&lb, &ub);
        if (res != NO_ERR) {
        ...
        simtyp = xml_val_new_struct(instance, XSD_SIM_TYP, xsd_id);
        if (!simtyp) {
            val_free_value(instance, top);
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, simtyp, top);
        }

        /* need a restriction wrapper */
        rest = new_restriction(instance, mod, list_id, typname, TRUE);
        if (!rest) {
            val_free_value(instance, top);
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, rest, simtyp);
        }

        /* check fixed length or a range given */
        ret = ncx_compare_nums(instance, lb, ub, range_btyp);
        if (ret) {
            /* range given, gen lower bound if not zero */
            if (!ncx_num_zero(instance, lb, range_btyp)) {
                lval = xml_val_new_flag(instance, XSD_MIN_LEN, xsd_id);
                if (!lval) {
                    res = ERR_INTERNAL_MEM;
                } else {
                    val_add_child(instance, lval, rest);
                    res = ncx_sprintf_num(instance, numbuff, lb, range_btyp, &len);
                    if (res == NO_ERR) {
                        res = xml_val_add_cattr(instance, XSD_VALUE, 0, numbuff, lval);
                    }
                }
                if (res != NO_ERR) {
                    val_free_value(instance, top);
            ...
            /* gen the upper bound */
            lval = xml_val_new_flag(instance, XSD_MAX_LEN, xsd_id);
            if (!lval) {
                res = ERR_INTERNAL_MEM;
            } else {
                val_add_child(instance, lval, rest);
                res = ncx_sprintf_num(instance, numbuff, ub, range_btyp, &len);
                if (res == NO_ERR) {
                    res = xml_val_add_cattr(instance, XSD_VALUE, 0, numbuff, lval);
                }
            }
            ...
            lval = xml_val_new_flag(instance, XSD_LENGTH, xsd_id);
            if (!lval) {
                res = ERR_INTERNAL_MEM;
            } else {
                val_add_child(instance, lval, rest);
                res = xml_val_add_cattr(instance, XSD_FIXED, 0, XSD_TRUE, lval);
                if (res == NO_ERR) {
                    res = ncx_sprintf_num(instance, numbuff, lb, range_btyp, &len);
                    if (res == NO_ERR) {
                        res = xml_val_add_cattr(instance, XSD_VALUE, 0, numbuff, lval);
    ...
    val_add_child(instance, top, val);
    return NO_ERR;
...
*********************************************************************/
static status_t
    finish_attributes (ncx_instance_t *instance,
                       const ncx_module_t *mod,
                       const typ_def_t *typdef,
                       val_value_t *val)
{
    ...
    for (typ_attr = typ_first_meta_con(typdef);
         typ_attr != NULL;
         typ_attr = (typ_child_t *)dlq_nextEntry(instance, typ_attr)) {

        /* get an attribute node */
        aval = xml_val_new_flag(instance, XSD_ATTRIBUTE, xsd_id);
        if (!aval) {
            return ERR_INTERNAL_MEM;
        }

        /* set the attribute name */
        res = xml_val_add_attr(instance, NCX_EL_NAME, 0, typ_attr->name, aval);
        if (res != NO_ERR) {
            val_free_value(instance, aval);
            return res;
        }

        /* set the data type */
        res = xsd_add_type_attr(instance, mod, &typ_attr->typdef, aval);
        if (res != NO_ERR) {
            val_free_value(instance, aval);
            return res;
        }
        ...
        iqual = typ_get_iqualval_def(instance, &typ_attr->typdef);
        switch (iqual) {
        case NCX_IQUAL_ONE:
        case NCX_IQUAL_1MORE:
            res = xml_val_add_cattr(instance, XSD_USE, 0, XSD_REQUIRED, aval);
            break;
        case NCX_IQUAL_OPT:
        ...
        name = typ_get_default(instance, &typ_attr->typdef);
        if (name) {
            res = xml_val_add_cattr(instance, NCX_EL_DEFAULT, 0, name, aval);
            if (res != NO_ERR) {
                val_free_value(instance, aval);
                return res;
            }
         ...
         */
        val_add_child(instance, aval, val); 
    }
...
*********************************************************************/
static status_t
    finish_simpleMetaType (ncx_instance_t *instance,
                           const ncx_module_t *mod,
                           ...
                           val_value_t *val)
{
    ...
    simcon = xml_val_new_struct(instance, XSD_SIM_CON, xsd_id);
    if (!simcon) {
        return ERR_INTERNAL_MEM;
    }
    ...
    if (!ext) {
        val_free_value(instance, simcon);
        return ERR_INTERNAL_MEM;
    }
    val_add_child(instance, ext, simcon);   /* add early */

    basename = xsd_make_basename(instance, typ);
    if (!basename) {
        val_free_value(instance, simcon);
        return ERR_INTERNAL_MEM;
    }
     ...
     */
    res = xml_val_add_attr(instance, XSD_BASE, 0, basename, ext);
    if (res != NO_ERR) {
        m__free(instance, basename);
        val_free_value(instance, simcon);
        return ERR_INTERNAL_MEM;
    }
    ...
    res = finish_attributes(instance, mod, &typ->typdef, ext);
    if (res != NO_ERR) {
        val_free_value(instance, simcon);
        return ERR_INTERNAL_MEM;
    }
    ...
    val_add_child(instance, simcon, val); 
    return NO_ERR;
...
*********************************************************************/
status_t
    xsd_add_types (ncx_instance_t *instance,
                   const ncx_module_t *mod,
                   val_value_t *val)
{
    typ_template_t         *typ;
    ...
    for (typ = (typ_template_t *)dlq_firstEntry(instance, &mod->typeQ);
         ...
         */
        sim = typ_is_xsd_simple(instance, btyp);

        if (sim) {
            tval = xml_val_new_struct(instance, XSD_SIM_TYP, xsd_id);
        } else {
            tval = xml_val_new_struct(instance, XSD_CPX_TYP, xsd_id);
        }
        if (!tval) {
            return ERR_INTERNAL_MEM;
        }
        ...
        if (base) {
            basename = xsd_make_basename(instance, typ);
            if (!basename) {
                val_free_value(instance, tval);
                return ERR_INTERNAL_MEM;
            }
            res = xml_val_add_attr(instance, NCX_EL_NAME, 0, basename, tval);
            if (res != NO_ERR) {
                m__free(instance, basename);
            }
        } else {
            res = xml_val_add_cattr(instance, NCX_EL_NAME, 0, typ->name, tval);
        }
        if (res != NO_ERR) {
            val_free_value(instance, tval);
            return res;
        ...
        if (base) {
            ...
            if (tclass==NCX_CL_NAMED) {
                res = xsd_finish_namedType(instance, mod, &typ->typdef, tval);
            } else if (sim) {
                res = xsd_finish_simpleType(instance, mod, &typ->typdef, tval);
            }
            if (res != NO_ERR) {
                val_free_value(instance, tval);
                return res;
            } else {
                val_add_child(instance, tval, val);
            }
         ...
         */
        res = xsd_do_type_annotation(instance, typ, tval);
        if (res != NO_ERR) {
            val_free_value(instance, tval);
            return res;
        }
        ...
        if (base) {
            res = finish_simpleMetaType(instance, mod, typ, tval);
...
*********************************************************************/
status_t
    xsd_add_local_types (ncx_instance_t *instance,
                         const ncx_module_t *mod,
                         val_value_t *val)
{
    ...
    for (typnam = (ncx_typname_t *)dlq_firstEntry(instance, &mod->typnameQ);
         typnam != NULL;
         typnam = (ncx_typname_t *)dlq_nextEntry(instance, typnam)) {
        ...
        res = xml_val_add_cattr(instance, NCX_EL_NAME, 0, typnam->typname, tval);
        if (res != NO_ERR) {
            return res;
        }
...
*********************************************************************/
status_t
    xsd_finish_simpleType (ncx_instance_t *instance,
                           const ncx_module_t *mod,
                           typ_def_t *typdef,
                           val_value_t *val)
{
    ...
    xsd_id = xmlns_xs_id(instance);
    btyp = typ_get_basetype(instance, typdef);
    typename = xsd_typename(instance, btyp);
    range  = typ_get_crange_con(instance, typdef);
    rangeQ = typ_get_crangeQ_con(instance, typdef);
    rangecnt = (rangeQ) ? dlq_count(instance, rangeQ) : 0;
    typ_rdef = typ_first_rangedef_con(instance, typdef);
    res = NO_ERR;

    if (rangecnt > 1) {
        chval = new_range_union(instance, mod, range,
                                rangeQ, typdef,
                                xsd_id, typename);
        if (!chval) {
            return ERR_INTERNAL_MEM;
    ...
    switch (btyp) {
    case NCX_BT_ENUM:
        chval = make_enum(instance, mod, typdef);
        if (!chval) {
            return ERR_INTERNAL_MEM;
        } else {
    ...
    case NCX_BT_INSTANCE_ID:
        chval = new_restriction(instance, mod, xsd_id, typename,
                                    (rangecnt) ? TRUE : FALSE);
        if (!chval) {
            return ERR_INTERNAL_MEM;
        } else {
        ...
        if (rangecnt) {
            res = finish_range(instance, typdef, range, typ_rdef, chval);
            if (res != NO_ERR) {
                return res;
            }
        }
    ...
    case NCX_BT_STRING:
        ...
        case NCX_SR_PATTERN:
            ...
            patterncnt = typ_get_pattern_count(instance, typdef);
            if (patterncnt == 0) {
                return SET_ERROR(instance, ERR_INTERNAL_VAL);
            }

            /* build a restriction element */
            chval = new_restriction(instance, mod, xsd_id, 
            ...
            /* create the pattern element */
            res = add_patterns(instance, mod, typdef, xsd_id, NCX_EL_STRING, chval);
            if (res != NO_ERR) {
                return res;
            }
        ...
        break;
    case NCX_BT_SLIST:
        res = finish_list(instance, mod, btyp, typdef, val);
        break;
    case NCX_BT_UNION:
        res = xsd_finish_union(instance, mod, typdef, val);
        break;
    case NCX_BT_LEAFREF:
    case NCX_BT_IDREF:
        break;
    case NCX_BT_BITS:
        ...
        topcon = xml_val_new_struct(instance, NCX_EL_LIST, xsd_id);
        if (!topcon) {
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, topcon, val);  /* add early */
        }
        ...
        if (!simtyp) {
            ...
            val_add_child(instance, simtyp, topcon);  /* add early */
        }
        ...
        if (!chval) {
            ...
            val_add_child(instance, chval, simtyp);
        }
        break;
    case NCX_BT_EMPTY:
         ...
         */
        topcon = new_restriction(instance, mod, xsd_id, typename, TRUE);
        if (!topcon) {
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, topcon, val);
        }
        chval = xml_val_new_flag(instance, XSD_MAX_LEN, xsd_id);
        if (!chval) {
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, chval, topcon);
        }
        res = xml_val_add_cattr(instance, NCX_EL_VALUE, 0, XSD_ZERO, chval);
        break;
    default:
        return SET_ERROR(instance, ERR_INTERNAL_VAL);
    }
...
*********************************************************************/
status_t
    xsd_finish_namedType (ncx_instance_t *instance,
                          const ncx_module_t *mod,
                          typ_def_t *typdef,
                          val_value_t *val)
{
    ...
    res = NO_ERR;
    xsd_id = xmlns_xs_id(instance);
    btyp = typ_get_basetype(instance, typdef);            /* NCX base type */
    rdef = typ_first_rangedef_con(instance, typdef);        /* range def Q */
    range = typ_get_crange_con(instance, typdef);
    mdef = NULL;                /*** meta-data not checked yet ***/
    sdef = typ_first_strdef(instance, typdef);         /* string Q */
    patcnt = typ_get_pattern_count(instance, typdef);         /* pattern Q */
    issim = typ_is_xsd_simple(instance, btyp);           /* container type */
    typename = typ_get_named_typename(instance, typdef);
    ...
    if (!issim) {
        topcon = xml_val_new_struct(instance, XSD_CPX_CON, xsd_id);
        if (!topcon) {
            return ERR_INTERNAL_MEM;
        } else {
            val_add_child(instance, topcon, val);      /* add early */
    ...
    if (!isext && rdef) {
        rangeQ = typ_get_crangeQ_con(instance, typdef);
        if (dlq_count(instance, rangeQ) > 1) {
            chval = new_range_union(instance, mod, range,
                                    rangeQ, typdef,
                                    test_id, typename);
            if (!chval) {
    ...
    if (isext) {
        if (hasnodes) {
            chval = xml_val_new_struct(instance, XSD_EXTENSION, xsd_id);
        } else {
            chval = xml_val_new_flag(instance, XSD_EXTENSION, xsd_id);
        }
    } else {
        if (hasnodes) {
            chval = xml_val_new_struct(instance, XSD_RESTRICTION, xsd_id);
        } else {
            chval = xml_val_new_flag(instance, XSD_RESTRICTION, xsd_id);
        }
    }
    if (!chval) {
        return ERR_INTERNAL_MEM;
    } else {
        val_add_child(instance, chval, topcon);  /* add early */
    }
    ...
    if (patcnt <= 1) {
        if (test_id == mod->nsid) {
            ...
            res = xml_val_add_cattr(instance, XSD_BASE, 0, typename, chval);
            if (res != NO_ERR) {
                return ERR_INTERNAL_MEM;
            }
        } else if (!test_id) {
            ...
            res = xml_val_add_cattr
                (instance, XSD_BASE, 0, ncx_find_typname(instance, 
            ...
            str = xml_val_make_qname(instance, test_id, typename);
            if (!str) {
                return ERR_INTERNAL_MEM;
            }
            res = xml_val_add_attr(instance, XSD_BASE, 0, str, chval);
            if (res != NO_ERR) {
                m__free(instance, str);
                return ERR_INTERNAL_MEM;
            }
    ...
    if (hasnodes && typdef->def.named.newtyp) {
        res = NO_ERR;
        if (!isext && patcnt) {
            res = add_patterns(instance, mod, typdef,
                               test_id, typename, chval);
        }

        /* check if this is an range restriction */
        if (res == NO_ERR && !isext && rdef) {
            res = finish_range(instance, typdef, range, rdef, chval);
        }
        ...
        if (res==NO_ERR && isext && mdef) {
            ...
            res = finish_attributes(instance, mod, typdef->def.named.newtyp, chval);
        } 
#endif
    }
...
*********************************************************************/
status_t
    xsd_finish_union (ncx_instance_t *instance,
                      const ncx_module_t *mod,
                      typ_def_t *typdef,
                      val_value_t *val)
{
    ...
    while (un && simple_form) {
        if (un->typ) {
             ...
             */
            union_id = typ_get_nsid(instance, un->typ);
            if (!union_id) {
                union_id = mod->nsid;
            }
            typname = un->typ->name;
            ...
            if (union_id != mod->nsid) {        
                len += xml_val_qname_len(instance, union_id, typname) + 1;
            } else {
                len += xml_strlen(instance, typname) + 1;
            }
    ...
    if (simple_form) {
        ...
        top = xml_val_new_flag(instance, XSD_UNION, xsd_id);
        if (!top) {
            return ERR_INTERNAL_MEM;
        }
        ...
        while (un) {
            ...
            if (union_id != mod->nsid) {        
                siz = xml_val_sprintf_qname(instance, p, len, union_id, typname);
            } else {
                siz = xml_strcpy(instance, p, typname);
            }
            p += siz;
        ...
        *--p = 0;
        ...
        /* add the itemType attribute to the list element */
        res = xml_val_add_attr(instance, XSD_MEMBERTYPES, 0, str, top);
        if (res != NO_ERR) {
            m__free(instance, str);
            val_free_value(instance, top);
            return res;
         ...
         */
        top = xml_val_new_struct(instance, XSD_UNION, xsd_id);
        if (!top) {
            return ERR_INTERNAL_MEM;
        }
        ...
        while (un && res==NO_ERR) {
            ...
            if (!simtyp) {
                ...
                val_add_child(instance, simtyp, top);  /* add early */
            }

            untypdef = un->typdef;

            if (untypdef->tclass == NCX_CL_NAMED) {
                res = xsd_finish_namedType(instance, mod, untypdef, simtyp);
            } else {
                res = xsd_finish_simpleType(instance, mod, untypdef, simtyp);
            }
            if (res == NO_ERR) {
                un = (typ_unionnode_t *)dlq_nextEntry(instance, un);
            }
