...
*********************************************************************/
static void
    free_msg (ncx_instance_t *instance, rpc_msg_t *msg)
{
...
*********************************************************************/
static void
    send_rpc_error_info (ncx_instance_t *instance,
                         ses_cb_t *scb,
                         xml_msg_hdr_t  *msg,
                         ...
                         int32  indent)
{
    ...
    if (dlq_empty(instance, &err->error_info)) {
        return;
    }
    ...
    for (errinfo = (rpc_err_info_t *)
             dlq_firstEntry(instance, &err->error_info);
         errinfo != NULL;
         errinfo = (rpc_err_info_t *)dlq_nextEntry(instance, errinfo)) {
        ...
        case NCX_BT_SLIST:
            ...
            break;
        default:
            if (typ_is_simple(instance, errinfo->val_btype)) {
                SET_ERROR(instance, ERR_INTERNAL_VAL);
    ...
    xml_wr_end_elem(instance, scb, msg, ncid, NCX_EL_ERROR_INFO, indent);
...
*********************************************************************/
static status_t
    send_rpc_error (ncx_instance_t *instance,
                    ses_cb_t *scb,
                    xml_msg_hdr_t *msg,
                    const rpc_err_rec_t *err,
                    int32 indent)
{
    ...
    for (errinfo = (rpc_err_info_t *)
         ...
         errinfo != NULL;
         errinfo = (rpc_err_info_t *)dlq_nextEntry(instance, errinfo)) {
        res = xml_msg_check_xmlns_attr(instance,
                                       msg,
                                       errinfo->name_nsid,
                                       errinfo->badns,
                                       &attrs);
    ...
    send_rpc_error_info(instance, scb, msg,  err, indent);
    ...
    xml_wr_end_elem(instance, scb, msg, ncid, NCX_EL_RPC_ERROR, indent);
...
*********************************************************************/
static status_t
    check_add_changed_since_attr (ncx_instance_t *instance, rpc_msg_t *msg)
{
    const xmlChar *rpcname;
    xmlns_id_t   ncid = xmlns_nc_id(instance);

    if (msg->rpc_method == NULL) {
        return NO_ERR;
    }

    if (obj_get_nsid(instance, msg->rpc_method) != ncid) {
        /* not the NETCONF module */
        return NO_ERR;
    }

    rpcname = obj_get_name(instance, msg->rpc_method);

    /* check if <get> or <get-config> */
    if (!xml_strcmp(instance, rpcname, NCX_EL_GET) ||
        !xml_strcmp(instance, rpcname, NCX_EL_GET_CONFIG)) {
        ...
        attr = xml_find_attr_q(instance, msg->rpc_in_attrs, 0, NCX_EL_LAST_MODIFIED);
        if (attr != NULL) {
            return NO_ERR;
        }

        /* make a new attr and add it to rpc_in_attrs */
        res = xml_add_attr(instance, msg->rpc_in_attrs, 0, NCX_EL_LAST_MODIFIED,
                           cfg->last_ch_time);
...
*********************************************************************/
static void
    send_rpc_reply (ncx_instance_t *instance,
                    ses_cb_t *scb,
                    rpc_msg_t  *msg)
{
    ...
    errsend = !dlq_empty(instance, &msg->mhdr.errQ);

    res = xml_msg_clean_defns_attr(instance, msg->rpc_in_attrs);

    if (res == NO_ERR) {
        res = check_add_changed_since_attr(instance, msg);
    }

    if (res == NO_ERR) { 
        res = xml_msg_gen_xmlns_attrs(instance, &msg->mhdr, msg->rpc_in_attrs, errsend);
    }
    ...
    if (msg->rpc_method) {
        rpcid = obj_get_nsid(instance, msg->rpc_method);
    } else {
        rpcid = ncid;
    }
    ...
    datasend = (!dlq_empty(instance, &msg->rpc_dataQ) 
                || msg->rpc_datacb) ? TRUE : FALSE;

    /* check which reply variant needs to be sent */
    if (dlq_empty(instance, &msg->mhdr.errQ) && !datasend) {
        /* no errors and no data, so send <ok> variant */
        xml_wr_empty_elem(instance, scb, &msg->mhdr, ncid, ncid, NCX_EL_OK, indent);
    } else {
        ...
        for (err = (const rpc_err_rec_t *)
                 dlq_firstEntry(instance, &msg->mhdr.errQ);
             err != NULL;
             err = (const rpc_err_rec_t *)dlq_nextEntry(instance, err)) {

            res = send_rpc_error(instance, scb, &msg->mhdr, err, indent);
            if (res != NO_ERR) {
                SET_ERROR(instance, res);
            }
        }
        ...
        if (datasend) {
            if (msg->rpc_data_type == RPC_DATA_STD) {
                xml_wr_begin_elem(instance, scb, &msg->mhdr, ncid, rpcid, NCX_EL_DATA, 
                                  indent);
                if (indent > 0) {
                    indent += ses_indent_count(scb);
                }
            ...
            if (msg->rpc_datacb) {
                ...
                res = (*agtcb)(instance, scb, msg, indent);
                if (res != NO_ERR) {
                    log_error(instance,
                ...
                for (val = (val_value_t *)
                         dlq_firstEntry(instance, &msg->rpc_dataQ);
                     val != NULL;
                     val = (val_value_t *)dlq_nextEntry(instance, val)) {

                    xml_wr_full_val(instance, scb, &msg->mhdr, val, indent);
                }
            }
            ...
            if (msg->rpc_data_type == RPC_DATA_STD) {
                ...
                xml_wr_end_elem(instance, scb, &msg->mhdr, rpcid, NCX_EL_DATA, 
    ...
    xml_wr_end_elem(instance, scb, &msg->mhdr, ncid, NCX_EL_RPC_REPLY, 0);
...
static obj_template_t *
    find_rpc (ncx_instance_t *instance,
              ...
              const xmlChar *rpcname)
{
    ...
    if (mod) {
        rpc = ncx_find_rpc(instance, mod, rpcname);
    }
    return rpc;
...
*********************************************************************/
static status_t
    parse_rpc_input (ncx_instance_t *instance,
                     ses_cb_t *scb,
                     rpc_msg_t  *msg,
                     ...
                     xml_node_t  *method)
{
    ...
    obj = obj_find_template(instance, obj_get_datadefQ(instance, rpcobj), NULL, YANG_K_INPUT);
    if (obj && obj_get_child_count(instance, obj)) {
        msg->rpc_agt_state = AGT_RPC_PH_PARSE;
        res = agt_val_parse_nc(instance, scb, &msg->mhdr, obj, method, NCX_DC_CONFIG, 
                               msg->rpc_input);

        if (LOGDEBUG3) {
            log_debug3(instance, "\nagt_rpc: parse RPC input state");
            rpc_err_dump_errors(instance, msg);
...
*********************************************************************/
static status_t
    post_psd_state (ncx_instance_t *instance,
                    ses_cb_t *scb,
                    rpc_msg_t  *msg,
                    status_t  psdres)
{
    ...
    if (psdres == NO_ERR) {
         ...
         ***/
        res = val_add_defaults(instance, msg->rpc_input, NULL, NULL, FALSE);
    }
    ...
    if (res == NO_ERR) {
         ...
         */
        res = agt_val_rpc_xpath_check(instance, scb, msg, &msg->mhdr, msg->rpc_input,
                                      msg->rpc_method);
    }
    ...
    if (res == NO_ERR) {
         ...
         */
        res = agt_val_instance_check(instance, scb, &msg->mhdr, msg->rpc_input, 
...
*********************************************************************/
static status_t
    load_config_file (ncx_instance_t *instance,
                      ...
                      dlq_hdr_t *errorQ)
{
    ...
    obj_template_t *rpcobj = find_rpc(instance, NC_MODULE, NCX_EL_LOAD_CONFIG);
    if (!rpcobj) {
    ...
    if (agt_cfg_txid_in_progress(instance, NCX_CFGID_RUNNING) && use_sid) {
        return SET_ERROR(instance, ERR_INTERNAL_INIT_SEQ);
    }

    /* create a dummy session control block */
    ses_cb_t *scb = agt_ses_new_dummy_session(instance);
    if (!scb) {
    ...
    status_t res = agt_ses_set_dummy_session_acm(instance, scb, use_sid);
    if (res != NO_ERR) {
    ...
    rpc_msg_t *msg = rpc_new_msg(instance);
    if (!msg) {
        agt_ses_free_dummy_session(instance, scb);
        return ERR_INTERNAL_MEM;
    }
    ...
    res = xml_get_reader_from_filespec(instance, (const char *)filespec, &scb->reader);
    if (res != NO_ERR) {
        free_msg(instance, msg);
        agt_ses_free_dummy_session(instance, scb);
        return res;
    }
    ...
    xml_node_t  method;
    xml_init_node(instance, &method);
    method.nodetyp = XML_NT_START;
    method.nsid = xmlns_nc_id(instance);
    method.module = NC_MODULE;

    method.qname = xml_strdup(instance, NCX_EL_LOAD_CONFIG);
    ...
    res = parse_rpc_input(instance, scb, msg, rpcobj, &method);
    if (res != NO_ERR) {
        retres = res;
    }
    if (!(NEED_EXIT(res) || res==ERR_XML_READER_EOF)) {
         ...
         */
        res = post_psd_state(instance, scb, msg, res);
        if (res != NO_ERR) {
            retres = res;
        }
    }
    ...
    if (res == NO_ERR && cbset->acb[AGT_RPC_PH_VALIDATE]) {
        ...
        res = (*cbset->acb[AGT_RPC_PH_VALIDATE])(instance, scb, msg, &method);
        if (res != NO_ERR) {
            retres = res;
        } else {
            valdone = TRUE;
     ...
     */
    if (valdone) {
        val_value_t *testval = val_find_child(instance, msg->rpc_input, NULL, 
                                              NCX_EL_CONFIG);
        if (testval) {
            ...
            if (justval) {
                ...
                val_remove_child(instance, testval);
                *configval = testval;
            }
        }
    }
    ...
    if (!justval && valdone && cbset->acb[AGT_RPC_PH_INVOKE]) {
        ...
        res = (*cbset->acb[AGT_RPC_PH_INVOKE])(instance, scb, msg, &method);
        if (res != NO_ERR) {
    ...
    if (LOGDEBUG) {
        if (!dlq_empty(instance, &msg->mhdr.errQ)) {
            rpc_err_dump_errors(instance, msg);
        }
    }

    /* move any error messages to the config error Q */
    dlq_block_enque(instance, &msg->mhdr.errQ, errorQ);

    /* cleanup and exit */
    xml_clean_node(instance, &method);
    free_msg(instance, msg);
    agt_ses_free_dummy_session(instance, scb);
...
*********************************************************************/
status_t 
    agt_rpc_init (ncx_instance_t *instance)
{
    status_t  res;

    if (!agt_rpc_init_done) {
        res = top_register_node(instance, NC_MODULE, NCX_EL_RPC, agt_rpc_dispatch);
        if (res != NO_ERR) {
            return res;
        }
...
*********************************************************************/
void 
    agt_rpc_cleanup (ncx_instance_t *instance)
{
    if (agt_rpc_init_done) {
        top_unregister_node(instance, NC_MODULE, NCX_EL_RPC);
...
*********************************************************************/
status_t 
    agt_rpc_register_method (ncx_instance_t *instance,
                             ...
                             agt_rpc_method_t method)
{
    ...
    rpcobj = find_rpc(instance, module, method_name);
    if (!rpcobj) {
    ...
    if (rpcobj->cbset) {
        ...
        cbset = m__getObj(instance, agt_rpc_cbset_t);
        if (!cbset) {
...
*********************************************************************/
void 
    agt_rpc_support_method (ncx_instance_t *instance,
                            const xmlChar *module,
                            const xmlChar *method_name)
{
    obj_template_t  *rpcobj;
...
*********************************************************************/
void 
    agt_rpc_unsupport_method (ncx_instance_t *instance,
                              const xmlChar *module,
                              const xmlChar *method_name)
{
    obj_template_t  *rpcobj;
...
*********************************************************************/
void 
    agt_rpc_unregister_method (ncx_instance_t *instance,
                               const xmlChar *module,
                               const xmlChar *method_name)
{
    obj_template_t       *rpcobj;
...
*********************************************************************/
void 
    agt_rpc_dispatch (ncx_instance_t *instance,
                      ses_cb_t *scb,
                      xml_node_t *top)
{
    rpc_msg_t             *msg;
    ...
    if (scb->type != SES_TYP_DUMMY && scb->state != SES_ST_IDLE) {
        ...
        agt_ses_request_close(instance, scb, scb->sid, SES_TR_DROPPED);
    ...
    if (top->nodetyp != XML_NT_START) {
        ...
        agt_ses_request_close(instance, scb, scb->sid, SES_TR_OTHER);
        free_msg(instance, msg);
        return;
    }
    ...
    msg->mhdr.withdef = ses_withdef(instance, scb);

    /* get the NC RPC message-id attribute; must be present */
    attr = xml_find_attr(instance, top, 0, NCX_EL_MESSAGE_ID);
    ...
    if (!attr || !attr->attr_val) {
        ...
        agt_record_attr_error(instance, 
                              scb, 
                              &msg->mhdr, 
                              NCX_LAYER_RPC,
                              res2, 
     ...
     */
    res = xml_msg_build_prefix_map(instance, 
                                   &msg->mhdr, 
                                   msg->rpc_in_attrs, 
                                   TRUE, 
                                   !dlq_empty(instance, &msg->mhdr.errQ));

    /* init agt_acm cache struct even though it may not
     * get used if the RPC method is invalid
     */
    if (res == NO_ERR) {
        res = agt_acm_init_msg_cache(instance, scb, &msg->mhdr);
    }
    ...
    if (res != NO_ERR || res2 != NO_ERR) {
        send_rpc_reply(instance, scb, msg);
    ...
    xml_init_node(instance, &method);
    res = agt_xml_consume_node(instance, scb, &method, NCX_LAYER_RPC, &msg->mhdr);
    if (res != NO_ERR) {
        errdone = TRUE;
    } else {
        errdone = FALSE;
        ...
        if (!(method.nodetyp==XML_NT_START || 
            ...
            /* look for the RPC method in the definition registry */
            rpcobj = find_rpc(instance, method.module, method.elname);
            msg->rpc_method = rpcobj;
            ...
            if (!rpc) {
                ...
                res = ERR_NCX_OPERATION_NOT_SUPPORTED;
                       ...
                       !agt_acm_rpc_allowed(instance, &msg->mhdr, scb->username, 
                                            rpcobj)) {
                res = ERR_NCX_ACCESS_DENIED;
            } else {
    ...
    if (res != NO_ERR && !errdone) {
        if (res != ERR_NCX_ACCESS_DENIED) {
            ...
            buff = m__getMem(instance, xml_strlen(instance, method.qname) 
                             + xml_strlen(instance, RPC_ROOT) + 2);
            if (buff) {
                xml_strcpy(instance, buff, RPC_ROOT);
                xml_strcat(instance, buff, (const xmlChar *)"/");
                xml_strcat(instance, buff, method.qname);
            }
        }
    ...
    xml_init_node(instance, &testnode);
    if (res == NO_ERR) {
        res = agt_xml_consume_node(instance, scb, &testnode, NCX_LAYER_RPC, &msg->mhdr);
    }
    if (res == NO_ERR) {
        if (LOGDEBUG2) {
            log_debug2(instance, 
        ...
        if (LOGDEBUG3) {
            xml_dump_node(instance, &testnode);
        }
        res = xml_endnode_match(instance, top, &testnode);
    }
    xml_clean_node(instance, &testnode);


    /* check that there is nothing after the <rpc> element */
    if (res == NO_ERR && !xml_docdone(instance, scb->reader)) {

        /* get all the extra nodes and report the errors */
        res = NO_ERR;
        while (res==NO_ERR) {
            /* do not add errors such as unknown namespace */
            res = agt_xml_consume_node(instance, scb, &testnode, NCX_LAYER_NONE, NULL);
            if (res==NO_ERR) {
                res = ERR_NCX_UNKNOWN_ELEMENT;
                errstr = (char *)xml_strdup(instance, (const xmlChar *)RPC_ROOT);
                agt_record_error(instance, 
                                 scb, 
                                 &msg->mhdr, 
                                 NCX_LAYER_RPC, 
    ...
    if (res == NO_ERR) {
        testobj = obj_find_child(instance, rpcobj, NULL, YANG_K_INPUT);
        if (testobj && obj_get_child_count(instance, testobj)) {
            res = post_psd_state(instance, scb, msg, res);
        }
    }
    ...
    if ((res==NO_ERR) && (cbset && cbset->acb[AGT_RPC_PH_VALIDATE])) {
        ...
        if (res != NO_ERR) {
             ...
             */
            if (!rpc_err_any_errors(instance, msg)) {
                agt_record_error(instance, 
                                 scb, 
                                 &msg->mhdr, 
                                 NCX_LAYER_RPC, 
    ...
    if (res == NO_ERR) {
        res = xml_msg_finish_prefix_map(instance, 
                                        &msg->mhdr, 
                                        msg->rpc_in_attrs);
    }
    ...
    if (cbset && cbset->acb[AGT_RPC_PH_POST_REPLY]) {
        ...
        (void)(*cbset->acb[AGT_RPC_PH_POST_REPLY])(instance, scb, msg, &method);
    }

    /* check if there is any auditQ because changes to 
     * the running config were made
     */
    if (msg->rpc_txcb && !dlq_empty(instance, &msg->rpc_txcb->auditQ)) {
        agt_sys_send_sysConfigChange(instance, scb, &msg->rpc_txcb->auditQ);
    }
...
*********************************************************************/
status_t
    agt_rpc_load_config_file (ncx_instance_t *instance,
                              const xmlChar *filespec,
                              cfg_template_t  *cfg,
                              boolean isload,
                              ses_id_t  use_sid)
{
    ...
    res = load_config_file(instance, filespec, cfg, isload, use_sid, FALSE, NULL,
                           &cfg->load_errQ);
...
*********************************************************************/
val_value_t *
    agt_rpc_get_config_file (ncx_instance_t *instance,
                             const xmlChar *filespec,
                             ...
                             status_t *res)
{
    ...
    retval = NULL;
    *res = load_config_file(instance, filespec, targetcfg, FALSE, use_sid,  TRUE,
                            &retval, errorQ);
...
*********************************************************************/
status_t
    agt_rpc_fill_rpc_error (ncx_instance_t *instance,
                            const rpc_err_rec_t *err,
                            val_value_t *rpcerror)
{
    ...
    leafval = agt_make_leaf(instance,
                            ...
                            &res);
    if (leafval) {
        val_add_child(instance, leafval, rpcerror);
    } else {
        retres = res;
    }
    ...
    if (err->error_path) {
         ...
         * found   */
        boolean savewarn = ncx_warning_enabled(instance, ERR_NCX_NO_XPATH_CHILD);

        if (savewarn) {
            (void)ncx_turn_off_warning(instance, ERR_NCX_NO_XPATH_CHILD);
        }

        leafval = agt_make_leaf(instance, rpcerror->obj, NCX_EL_ERROR_PATH,
                                err->error_path, &res);
        ...
        if (savewarn) {
            (void)ncx_turn_on_warning(instance, ERR_NCX_NO_XPATH_CHILD);
        }
    }
...
*********************************************************************/
void
    agt_rpc_send_error_reply (ncx_instance_t *instance,
                              ses_cb_t *scb,
                              status_t retres)
{
    rpc_err_rec_t       *err;
    ...
    if (retres == ERR_NCX_INVALID_FRAMING) {
        ...
        layer = NCX_LAYER_RPC;
        error_path = xml_strdup(instance, RPC_ROOT);
        /* ignore malloc error; send as much as possible */
    }
        
    err = agt_rpcerr_gen_error(instance,
                               layer,
                               retres,
                               NULL,
                               NCX_NT_NONE,
    ...
    xml_init_attrs(instance, &attrs);
    xml_msg_init_hdr(instance, &mhdr);
    res = xml_msg_gen_xmlns_attrs(instance, &mhdr, &attrs, TRUE);
    if (res != NO_ERR) {
        if (err != NULL) {
        ...
        ses_finish_msg(instance, scb);
        xml_clean_attrs(instance, &attrs);
        xml_msg_clean_hdr(instance, &mhdr);
        return;
    }
    ...
    if (err != NULL) {
        res = send_rpc_error(instance, scb, &mhdr, err, indent);
    } else {
        log_error(instance,
