...
class OpenStackNodeDriver(NodeDriver, OpenStackDriverMixin):
    ...
    def list_sizes(self, location=None):
        """
        returns an array of flavors (sizes)
        """
        
        o = self.connection.request('/flavors/detail').object
        return self._to_sizes(o)
...
class OpenStack_1_0_NodeDriver(OpenStackNodeDriver):
    ...
    def _to_size(self, el):
        ...
        # should this retrieve the OS extras? 
...
class OpenStack_1_1_NodeDriver(OpenStackNodeDriver):
    ...
    #RIFT addition
    def ex_create_flavor(self, **kwargs):
        """     
        Create a new flavor
        ...
        resp = self.connection.request("/flavors",
                                       method='POST',
                                       data={'flavor': attributes})
        f  = self._to_size(resp.object['flavor'])
        if 'extra' in kwargs:
            resp = self.connection.request("/flavors/%s/os-extra_specs" % f.id,
                                       method='POST',
                                       data={'extra_specs': kwargs['extra'] }
                                       )
        return f

    #RIFT addition
    def ex_delete_flavor(self, flavor_id):
        """     
        delete flavor

        """     

        resp = self.connection.request('/flavors/%s' % (flavor_id),
                                       method='DELETE')
        return resp.status == httplib.ACCEPTED
        ...
        return Node(
            ...
            extra=dict(
                ...
                availability_zone=api_node.get('OS-EXT-AZ:availability_zone',
                                               None),
                host_name=api_node.get('OS-EXT-SRV-ATTR:host',
                                       None),
            )
        )
    ...
    def _to_size(self, api_flavor, price=None, bandwidth=None, extra=None ):
        ...
        if extra is None:
            extra = self._get_os_extra(api_flavor['id'])
    ...
    def _get_os_extra(self, size_id):
        try:
            o = self.connection.request('/flavors/%s/os-extra_specs' % size_id ).object
        except Exception as e:
            print('WARNING: Unable to get os-extra_specs for id %s' % size_id)
            return {}

        return o['extra_specs']
