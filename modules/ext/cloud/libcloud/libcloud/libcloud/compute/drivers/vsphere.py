...
class VSphereNodeDriver(NodeDriver):
    ...
    #RIFT addition
    @wrap_non_libcloud_exceptions
    def ex_create_node_from_template(self, name, template, resource_pool):
        """
        ...
        :param template: Specifies the name of the template 
        :type template: ``str``

        :param resource_pool: Specifies the name of the resource pool
        :type template: ``str``
        ...
        """
        server = self.connection.client
        template_vm = server.get_vm_by_name(template)
        rps = server.get_resource_pools()
        res_pool_mor = None
        if resource_pool is not None:	
            for mor, path in rps.iteritems():
                if re.match('.*%s' % resource_pool,path):
                    res_pool_mor = mor
        else:
            res_pool_mor = rps.keys()[0] 				    

        if res_pool_mor is None:
            raise ValueError('Invalid resource_pool specified for create_node')				    					 		

        clone_vm = template_vm.clone(name, True, None, res_pool_mor)	    
        new_node = self._to_node(vm=clone_vm)
    ...
    def _to_node(self, vm):
        ...
        try:
            ...
            resource_pool_id = 'unknown' # JLM not sure what a good default would be...
