...
class OpenStackIdentity_3_0_Connection(OpenStackIdentityConnection):
    ...
    def create_role(self, name):
        ...
        :return: Created role.
        :rtype: :class:`.OpenStackIdentityRole`
        """
        data = {
            'name': name,
        }

        data = json.dumps({'role': data})
        response = self.authenticated_request('/v3/roles', data=data,
                                              method='POST')

        role = self._to_role(data=response.object['role'])
        return role
    ...
    def delete_role(self, role):
        ...
        :rtype: ``bool``
        """
        resp = self.authenticated_request('/v3/roles/%s' % (role.id),
                                       method='DELETE')
    ...
    def create_project(self, name, description=None,
        ...
        :param domain_id: ID of the domain to add the project to (optional).
        :type domain_id: ``str``

        :param enabled: True to enable project after creation.
        :type enabled: ``bool``

        :return: Created project.
        :rtype: :class:`.OpenStackIdentityProject`
        """
        data = {
            'name': name,
            'enabled': enabled
        ...
        response = self.authenticated_request('/v3/projects', data=data,
                                              method='POST')

        project = self._to_project(data=response.object['project'])
        return project
    ...
    def delete_project(self, project):
        ...
        :param      project: project which should be deleted
        :type       project: :class:`OpenStackIdentityProject`

        :rtype: ``bool``
        """
        resp = self.authenticated_request('/v3/projects/%s' % (project.id),
                                       method='DELETE')
        return resp.status == httplib.NO_CONTENT
    ...
    def delete_user(self, user):
        ...
        :rtype: ``bool``
        """
        resp = self.authenticated_request('/v3/users/%s' % (user.id),
                                       method='DELETE')
        return resp.status == httplib.NO_CONTENT
