...
public class Vala.GirParser : CodeVisitor {
	...
	class Node {
		...
		public HashMap<string, Node> parents = new HashMap<string, Node> (str_hash, str_equal);
		...
		private string? cached_lower_case_cprefix = null;
		private string? cached_lower_case_csuffix = null;
		private string? cached_default_lower_case_cprefix = null;
		private string? cached_default_lower_case_csuffix = null;

		public Node parent {
			get { return _parent; }
			set {
				_parent = value;
				cached_lower_case_cprefix = null;
				cached_lower_case_csuffix = null;
				cached_default_lower_case_cprefix = null;
				cached_default_lower_case_csuffix = null;
			}
		}
		...
		public string get_lower_case_cprefix () {
			if (cached_lower_case_cprefix == null) {
				cached_lower_case_cprefix = compute_lower_case_cprefix ();
			}

			return cached_lower_case_cprefix;
		}

		private string compute_lower_case_cprefix () {
			if (name == null) {
				return "";
			}
		...
		public string get_default_lower_case_cprefix () {
			if (cached_default_lower_case_cprefix == null) {
				cached_default_lower_case_cprefix = compute_default_lower_case_cprefix ();
			}

			return cached_default_lower_case_cprefix;
		}

		private string compute_default_lower_case_cprefix () {
			return "%s%s_".printf (parent.get_lower_case_cprefix (), get_lower_case_csuffix ());
		}

		public string get_lower_case_csuffix () {
			if (cached_lower_case_csuffix == null) {
				cached_lower_case_csuffix = compute_lower_case_csuffix ();
			}

			return cached_lower_case_csuffix;
		}

		private string compute_lower_case_csuffix () {
		...
		public string get_default_lower_case_csuffix () {
			if (cached_default_lower_case_csuffix == null) {
				cached_default_lower_case_csuffix = compute_default_lower_case_csuffix ();
			}

			return cached_default_lower_case_csuffix;
		}

		private string compute_default_lower_case_csuffix () {
		...
		public void process (GirParser parser) {
			...
			if (symbol is Namespace && parent == parser.root) {
				...
				parser.process_namespace_parents (this);
	...
	void parse_namespace () {
		...
		if (current.new_symbol) {
			...
			ns.attributes = new HashMap<string, Attribute> (str_hash, str_equal);
	...
	void process_alias (Node alias) {
		...
		if (base_type == null || type_sym == null || type_sym is Struct) {
			...
			var attributes_iter = orig.attributes.map_iterator ();
			while (attributes_iter.next ()) {
				var attribute = attributes_iter.get_value ();
				deleg.attributes.set (attribute.name, attribute);
			}
	...
	void find_parent (string cname, Node current, ref Node best, ref int match) {
		...
		for (var i = name.length - 1; i > 0; i--) {
			name[i] = 0;
			var parent = current.parents.get((string) name);
			if (parent != null) {
				best = parent;
				match = parent.get_lower_case_cprefix ().length;
				return;
			}
		}
	}
	...
	void process_namespace_parents (Node ns) {
		foreach (var child in ns.members) {
			// symbol is null only for aliases that aren't yet processed
			if (child.symbol == null || is_container (child.symbol)) {
				ns.parents.set (child.get_lower_case_cprefix (), child);
			}
		}
	}
	...
	void process_async_method (Node node) {
		...
		if (finish_method_node != null && finish_method_node.symbol is Method) {
			...
			if (finish_method is CreationMethod) {
				...
				var attributes_iter = m.attributes.map_iterator ();
				while (attributes_iter.next ()) {
					var attribute = attributes_iter.get_value ();
					method.attributes.set (attribute.name, attribute);
				}
