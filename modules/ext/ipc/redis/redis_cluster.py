...
class RedisCluster(object):
  def __init__(self, startPort, dir, cleanup, numNodes, gemDir, daemonize = False):
    ...
    if (False == os.path.isfile(self.dir+"/redis-server")):
      print "Required {0} executable not found".format(self.dir+"/redis-server")
      #sys.exit(2)
  ...
  def runSingleInstance(self):
    ...
    pid = os.fork()
    if pid == 0:
      arg=[self.dir+"/redis-server", "--port "+str(self.startPort)]
      os.execv(self.dir + '/redis-server', arg)
      sys.exit(0)
    else:
  ...
  def startRedisServers(self):
    ...
    for val in range(self.numNodes):
      pid = os.fork()
      if pid == 0:
        arg=[self.dir+'/redis-server', 'redis.conf']
