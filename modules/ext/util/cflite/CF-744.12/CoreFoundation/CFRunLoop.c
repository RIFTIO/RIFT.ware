...
static kern_return_t mk_timer_arm(int timer, int64_t expire_time) {
    ...
    ts.it_value.tv_sec = expire_time / NSEC_PER_SEC;
    ts.it_value.tv_nsec = expire_time % NSEC_PER_SEC;
...
static CFRunLoopSourceRef __CFRunLoopModeFindSourceForPort(CFRunLoopRef rl, CFRunLoopModeRef rlm, __CFPort port) {        /* DOES CALLOUT */
    CHECK_FOR_FORK();
