 ...
 */
static const struct pci_device_id fm10k_pci_tbl[] __devinitconst = {
...
static void fm10k_watchdog_host_is_ready(struct fm10k_intfc *interface)
{
	...
	/* request enablement of Tx timestamps */
	fm10k_ts_request_tx_mode(interface);
...
static s32 fm10k_timestamp_mode_msg(struct fm10k_hw *hw, u32 **results,
				    struct fm10k_mbx_info __always_unused *mbx)
{
	struct fm10k_swapi_tx_timestamp_mode resp;
	struct fm10k_intfc *interface;
	u16 glort, mode;
	s32 err;

	err = fm10k_tlv_attr_get_le_struct(
				results[FM10K_PF_ATTR_ID_TIMESTAMP_MODE_RESP],
				&resp, sizeof(resp));
	if (err)
		return err;
	...
	glort = le16_to_cpu(resp.glort);
	if (!fm10k_glort_valid_pf(hw, glort))
		return FM10K_ERR_PARAM;

	interface = container_of(hw, struct fm10k_intfc, hw);

	if (glort != interface->glort)
		return FM10K_ERR_PARAM;

	/* currently we only support PEP->ANY timestamp mode */
	mode = le16_to_cpu(resp.maxmode);
	if (mode == FM10K_TIMESTAMP_MODE_PEP_TO_ANY) {
		if (!(interface->flags & FM10K_FLAG_TX_TS_ENABLED))
			netif_warn(interface, drv, interface->netdev,
				   "Switch enabled 1588 Tx timestamps\n");
		interface->flags |= FM10K_FLAG_TX_TS_ENABLED;
	} else {
		if (interface->flags & FM10K_FLAG_TX_TS_ENABLED)
			netif_warn(interface, drv, interface->netdev,
				   "Switch disabled 1588 Tx timestamps\n");
		interface->flags &= ~FM10K_FLAG_TX_TS_ENABLED;
	}
...
static const struct fm10k_msg_data pf_mbx_data[] = {
	...
	FM10K_PF_MSG_TIMESTAMP_MODE_HANDLER(fm10k_timestamp_mode_msg),
	FM10K_TLV_MSG_ERROR_HANDLER(fm10k_mbx_error),
};
				   ...
				   const struct pci_device_id *ent)
{
	...
	/* assign BAR 4 resources for use with UIO and PTP */
