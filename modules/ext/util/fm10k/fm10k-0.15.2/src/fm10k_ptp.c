...
void fm10k_ts_tx_enqueue(struct fm10k_intfc *interface, struct sk_buff *skb)
{
	...
	/* form GLORT pair based on either frame, or interface data */
	if (skb->protocol == htons(ETH_P_IES)) {
		u64 ftag = be64_to_cpup((__be64 *)skb->data);

		FM10K_CB(clone)->fi.ftag = cpu_to_le64(ftag);
	}
	...
	if (skb)
		...
		skb_shinfo(clone)->tx_flags.in_progress = 1;
#else
...
void fm10k_ts_reset(struct fm10k_intfc *interface)
{
	...
	interface->ptp_adjust = ns - fm10k_systime_read(interface);
...
void fm10k_ts_request_tx_mode(struct fm10k_intfc *interface)
{
	struct fm10k_mac_info *mac = &interface->hw.mac;

	mac->ops.request_tx_timestamp_mode(&interface->hw,
					   interface->glort,
					   FM10K_TIMESTAMP_MODE_PEP_TO_ANY);
}
	...
	case HWTSTAMP_TX_ON:
		if (!(interface->flags & FM10K_FLAG_TX_TS_ENABLED)) {
			netif_warn(interface, drv, interface->netdev,
				   "Switch has not enabled 1588 Tx timestamps\n");
			return -EINVAL;
		}
		break;
	default:
