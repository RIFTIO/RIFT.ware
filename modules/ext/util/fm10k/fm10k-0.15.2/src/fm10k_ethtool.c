			      ...
			      u32 stringset, u8 *data)
{
	...
	case ETH_SS_STATS:
		...
		if (iov_data)
			for (i = 0; i < iov_data->num_vfs; i++) {
				sprintf(p, "vf_%u_mbx_tx_busy", i);
				p += ETH_GSTRING_LEN;
				sprintf(p, "vf_%u_mbx_tx_oversized", i);
				p += ETH_GSTRING_LEN;
				sprintf(p, "vf_%u_mbx_tx_messages", i);
				p += ETH_GSTRING_LEN;
				sprintf(p, "vf_%u_mbx_tx_dwords", i);
				p += ETH_GSTRING_LEN;
				sprintf(p, "vf_%u_mbx_rx_messages", i);
				p += ETH_GSTRING_LEN;
				sprintf(p, "vf_%u_mbx_rx_dwords", i);
				p += ETH_GSTRING_LEN;
				sprintf(p, "vf_%u_mbx_rx_parse_err", i);
				p += ETH_GSTRING_LEN;
			}
#endif /* DEBUG_STATS */
	...
	case ETH_SS_PRIV_FLAGS:
		memcpy(p, fm10k_prv_flags,
		       ETH_GSTRING_LEN * FM10K_PRV_FLAG_LEN);
		break;
	}
}

static int fm10k_get_sset_count(struct net_device *dev,
				int sset)
{
	struct fm10k_intfc *interface = netdev_priv(dev);
				    ...
				    u64 *data)
{
	...
	if (iov_data)
		for (i = 0; i < iov_data->num_vfs; i++) {
			...
			*(data++) = vf_info ? vf_info->mbx.tx_busy : 0;
			*(data++) = vf_info ? vf_info->mbx.tx_dropped : 0;
			*(data++) = vf_info ? vf_info->mbx.tx_messages : 0;
			*(data++) = vf_info ? vf_info->mbx.tx_dwords : 0;
			*(data++) = vf_info ? vf_info->mbx.rx_messages : 0;
			*(data++) = vf_info ? vf_info->mbx.rx_dwords : 0;
			*(data++) = vf_info ? vf_info->mbx.rx_parse_err : 0;
		}
#endif /* DEBUG_STATS */
...
static u32 fm10k_get_priv_flags(struct net_device *netdev)
{
	...
	if (interface->flags & FM10K_FLAG_IES_MODE)
		priv_flags |= 1 << FM10K_PRV_FLAG_IES;
...
static int fm10k_set_priv_flags(struct net_device *netdev, u32 priv_flags)
{
	...
	if (priv_flags & (1 << FM10K_PRV_FLAG_IES)) {
			   ...
			   struct ethtool_ts_info *info)
{
	...
	/* we can't always support Tx timestamps */
	if (interface->flags & FM10K_FLAG_TX_TS_ENABLED) {
		info->so_timestamping |=
