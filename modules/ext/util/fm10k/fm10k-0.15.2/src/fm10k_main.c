...
#define DRV_VERSION	__stringify(0.15.2) DRV_HW_PERF
...
/**
 * fm10k_get_hw_dev - Return netdevice based on hw structure
 * @hw: Pointer to the hardware structure
 *
 * Used for feeding a netdev to hw_dbg
 **/
struct net_device *fm10k_get_hw_dev(struct fm10k_hw *hw)
{
	return ((struct fm10k_intfc *)hw->back)->netdev;
}
			     ...
			     struct sk_buff *skb)
{
	...
	skb->protocol = ies_type_trans(skb, dev);
#ifdef NETIF_F_HW_L2FW_DOFFLOAD
	...
	if (fm10k_skb_is_vlan_tagged(skb)) {
		if (*vlgrp) {
			...
			vlan_gro_receive(&q_vector->napi, *vlgrp,
					 ...
					 skb);
			return;
		} else if (FM10K_CB(skb)->fi.w.vlan != interface->hw.mac.default_vid) {
			/* this isn't the default vid, so drop it */
			dev_kfree_skb_any(skb);
			return;
		}
	}
...
static __be16 fm10k_tx_encap_offload(struct sk_buff *skb)
{
	 ...
	 */
	if (skb_inner_transport_header(skb) + inner_tcp_hdrlen(skb) -
...
static u8 fm10k_tx_desc_flags(struct sk_buff *skb, u32 tx_flags)
{
	...
	if (unlikely(skb_shinfo(skb)->tx_flags.hardware) &&
	    likely(skb_shinfo(skb)->tx_flags.in_progress))
#else
