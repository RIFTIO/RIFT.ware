 ...
 *  - 1 - FTAG interface for passing switch traffic with outer FTAG and timestamp
 *
 * Default Value: 0
 */
					   ...
					   struct fm10k_option *opt)
{
	...
	pr_info("fm10k: Invalid %s specified (%d),  %s\n",
 ...
 **/
void __devinit fm10k_check_options(struct fm10k_intfc *interface)
{
	struct fm10k_ring_feature *feature = interface->ring_feature;

	/* record interface index */
	interface->bd_number = ifc;
#ifndef RW_FM10K_DISABLED
...
#ifndef RW_FM10K_DISABLED
		if (num_max_vfs > vf_ifc) {
			vfs = max_vfs[vf_ifc];
			fm10k_validate_option(&vfs, &opt);
		}
                vf_ifc++;
#else
                if (num_max_vfs > ifc) {
