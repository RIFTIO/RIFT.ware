...
#if 0
        if (dev_info.netns_fd){
          dev_net_set(net_dev, get_net_ns_by_fd(dev_info.netns_fd));
        }
#else   
        if (&init_net != current->nsproxy->net_ns){
          dev_net_set(net_dev, current->nsproxy->net_ns);
          KNI_PRINT("Creating kernel interface %s (ifindex:%u) in network context\n",
                    net_dev->name, net_dev->ifindex);
        }else{
          KNI_PRINT("Creating kernel interface %s (ifindex: %u) in init_net\n",
                    net_dev->name, net_dev->ifindex);
        }
#endif
#endif
