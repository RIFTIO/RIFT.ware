                                             ...
                                             struct sk_buff *skb)
{
  ...
  pkt_type = skb->pkt_type;
   if (RW_KNI_VF_VALID_MDATA_KNI_FILTER_FLAG(mbuf)){
    if (RW_KNI_VF_GET_MDATA_KNI_FILTER_FLAG(mbuf) & RW_FPATH_PKT_KNI_TREAT_AS_TX){
      ...
      kni->rx_treat_as_tx++;
      if (RW_KNI_VF_GET_MDATA_KNI_FILTER_FLAG(mbuf) & RW_FPATH_PKT_KNI_DISCARD_ON_RX){
