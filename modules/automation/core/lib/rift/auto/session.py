...
class ProxyParseError(ProxyError):
    '''Exception class representing the failure to parse input documents'''
    pass
...
class Proxy(object):
    ...
    def _xml_strip_rpc_reply(xml):
        ...
        User defined RPC responses are wrapped in <rpc-reply>
        This method strips that container from the response.
        ...
        '''
        pos_reply_tag = xml.find(r'<rpc-reply', 0)
        pos_reply_start = xml.find(r'>', pos_reply_tag) + 1
        pos_reply_end = xml.rfind(r'</rpc-reply', pos_reply_start)
        if pos_reply_start >= 0 and pos_reply_end >= 0:
            xml = xml[pos_reply_start:pos_reply_end]
        elif xml.find(r'<rpc-reply/>', 0):
            xml = ""
        return xml

    @staticmethod
    def _xml_strip_standard_reply(xml):
        '''Strip standard rpc-reply xml from the supplied xml

        Standards defined RPC responses are wrapped in
        <rpc-reply><data> ... </data></rpc-reply>.
        This method strips those containers from the response.
    ...
    def _rpc_fix_root(xml, rpc_name, prefix, namespace):
        ...
        xml = xml.replace(Proxy.XML_DECLARATION, '')
        parser = etree.XMLParser(huge_tree=True)
        xml_root = etree.XML("<falseroot>%s</falseroot>" % xml.encode('ascii'), parser)
...
class NetconfProxy(Proxy):
    ...
    def _get_xml(self, xpath):
        ...
        return Proxy._xml_strip_standard_reply(response_xml)
