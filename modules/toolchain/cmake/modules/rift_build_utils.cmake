...
# Replacement for rift_find_checkedout_submodules for an opensourced platform build.

function(rift_find_checkedout_submodules_without_git)
  ...
  set(${ARGS_OUT_SUBMODULES} "modules/tools/scripts;modules/toolchain;modules/core/mgmt;modules/core/util;modules/core/rwvx;modules/automation/core;modules/ext/util;modules/ext/yang" PARENT_SCOPE)

endfunction(rift_find_checkedout_submodules_without_git)
...
function(rift_checkout_submodule)
  ...
  message("Checking out local branch on ${ARGS_SUBMODULE}")
  unset(_result_code)
  execute_process(
    COMMAND git xinit -s ${ARGS_SUBMODULE}
...
function(rift_fetch_submodule_cache)
  ...
  # Parse the function arguments
  set(parse_options "")
  set(parse_onevalargs PROJECT_TOP_DIR BUILD_CACHE_DIR BUILD_CACHE_TYPE SUBMODULE INSTALL_PREFIX OUT_FOUND)
  set(parse_multivalueargs "")
  cmake_parse_arguments(ARGS "${parse_options}" "${parse_onevalargs}" "${parse_multivalueargs}" ${ARGN})

  message("Fetching the cache for submodule ${ARGS_SUBMODULE}")
  rift_get_submodule_hash(
    PROJECT_TOP_DIR ${ARGS_PROJECT_TOP_DIR}
    SUBMODULE ${ARGS_SUBMODULE}
    OUT_HASH _hash)

  rift_cache_path(_cache ${ARGS_BUILD_CACHE_DIR} ${ARGS_BUILD_CACHE_TYPE} ${ARGS_SUBMODULE} ${_hash})

  if(EXISTS ${_cache})
    message("Cache exists for ${ARGS_BUILD_CACHE_DIR}/${ARGS_BUILD_CACHE_TYPE}/${ARGS_SUBMODULE}/${_hash}")
    set(${ARGS_OUT_FOUND} 1 PARENT_SCOPE)

    if(EXISTS ${ARGS_INSTALL_PREFIX}/${ARGS_SUBMODULE}/RPMS/cache_${ARGS_BUILD_CACHE_TYPE}_${_hash}.log)
      message("Cache already populated for ${ARGS_BUILD_CACHE_TYPE}/${ARGS_SUBMODULE}/${_hash}")
    else(EXISTS ${ARGS_INSTALL_PREFIX}/${ARGS_SUBMODULE}/RPMS/cache_${ARGS_BUILD_CACHE_TYPE}_${_hash}.log)
      # copy the rpms from the cache directory
      file(REMOVE_RECURSE ${ARGS_INSTALL_PREFIX}/${ARGS_SUBMODULE})
      file(MAKE_DIRECTORY ${ARGS_INSTALL_PREFIX}/${ARGS_SUBMODULE})
      file(GLOB _rpm_files ${_cache}/*)
      file(
        COPY ${_rpm_files}
        DESTINATION ${ARGS_INSTALL_PREFIX}/${ARGS_SUBMODULE}/RPMS
        )

      # extract the RPMS in the local directory
      file(GLOB _rpm_files ${ARGS_INSTALL_PREFIX}/${ARGS_SUBMODULE}/RPMS/*.rpm)

      foreach(_rpm_file ${_rpm_files})
        message("Extracting RPM:")
        message("    RPM file: ${_rpm_file}")
        message("    PREFIX:   ${ARGS_INSTALL_PREFIX}")

        # OUTPUT_* and ERROR_* variables are NOT specified
        # They are piped to the CMAKE process
        execute_process(
            COMMAND
            ${ARGS_PROJECT_TOP_DIR}/bin/extract_rpm.sh
            ${ARGS_INSTALL_PREFIX}
            ${_rpm_file}
            RESULT_VARIABLE _result_code
            WORKING_DIRECTORY ${ARGS_INSTALL_PREFIX}
          )

        if(_result_code)
          message("Error = ${_result_code}")
          message(FATAL_ERROR "Failed to extract rpm ${_rpm_file}")
        endif(_result_code)
      ...
      # this is a bit ugly - gobject introspection also has the
      # install prefix embedded. Adjust these to the new path
      execute_process(
        COMMAND
          ${CMAKE_COMMAND} -DCMAKE_INSTALL_PREFIX=${CMAKE_INSTALL_PREFIX}
          -DPKG_INSTALL_PREFIX=${CMAKE_INSTALL_PREFIX}
          -P ${RIFT_CMAKE_MODULE_DIR}/rift_adj_prefix.cmake
        RESULT_VARIABLE _result_code
        )
      if(${_result_code})
          message(FATAL_ERROR "Failed to run rift_adj_prefix")
      endif(${_result_code})

    endif(EXISTS ${ARGS_INSTALL_PREFIX}/${ARGS_SUBMODULE}/RPMS/cache_${ARGS_BUILD_CACHE_TYPE}_${_hash}.log)

  else(EXISTS ${_cache})
    message("Cache not found for ${ARGS_BUILD_CACHE_DIR}/${ARGS_BUILD_CACHE_TYPE}/${ARGS_SUBMODULE}/${_hash}")
...
function(rift_is_cacheable)
  ...
  if(_result_code)
    ...
    message(FATAL_ERROR "Error: ${_error}")
  endif(_result_code)
  ...
  if(_output)
    ...
    # message(FATAL_ERROR "ARGS_BUILD_CACHE_DIR=${ARGS_BUILD_CACHE_DIR} ARGS_BUILD_CACHE_TYPE=${ARGS_BUILD_CACHE_TYPE} _submodule=${_submodule} hash=${_hash}")
