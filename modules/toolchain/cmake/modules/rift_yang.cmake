...
function(rift_yangdump_command var)
  ...
  if(NOT ARG_YANGDUMP_EXE)
    if(RIFT_SUBMODULE_NAME STREQUAL ext/yang)
      ...
      set(ARG_YANGDUMP_EXE /usr/bin/yangdump)
    endif()
  endif()
...
function(rift_yangdump_shell target)
  ...
  add_custom_command(
    ...
    COMMAND echo "${RIFT_CMAKE_BIN_DIR}/envset.sh \\\\" >> ${ARG_NAME}
...
function(rift_pyang_command_parallel var)
  ...
  set(${var}
    ${RIFT_CMAKE_BIN_DIR}/pyang_parallel.sh
...
function(rift_confdc_command var)
  set(parse_options)
  set(parse_onevalargs OUT_DEPENDS_VAR)
  set(parse_multivalueargs ARGV ENVSET YANG_DIRS)
  cmake_parse_arguments(ARG "${parse_options}" "${parse_onevalargs}" "${parse_multivalueargs}" ${ARGN})

  set(depends)
  if(RIFT_SUBMODULE_NAME STREQUAL core/util)
    set(CONFDC_EXE ${RIFT_SUBMODULE_BINARY_ROOT}/confd/confd-build/bin/confdc)
    list(APPEND depends build_confd)
  else()
    set(CONFDC_EXE ${CMAKE_INSTALL_PREFIX}/usr/local/confd/bin/confdc)
  endif()
  
  list(APPEND depends ${CONFDC_EXE})

  set(${var}
    ${RIFT_CMAKE_BIN_DIR}/envset.sh
      ${ARG_ENVSET}
      -e ${CONFDC_EXE} ${ARG_ARGV}
    PARENT_SCOPE
  )
  if(ARG_OUT_DEPENDS_VAR)
    set(${ARG_OUT_DEPENDS_VAR} ${depends} PARENT_SCOPE)
...
#   NO_LIB_COMPONENT        # Do not install to YLIB, instead install to COMPONENT 
...
function(rift_add_yang_target)
  ...
  if (ARG_NO_LIB_COMPONENT)
    set(library_pkg_name ${ARG_COMPONENT})
  else()
    set(library_pkg_name ${ARG_TARGET}_ylib-1.0)
  endif()
  ...
  if(exitstatus)
    message(FATAL_ERROR "Error: Failed to compute Yang dependencies: ${pycommand}")
  endif()
  ...
  foreach(yang_file ${ARG_YANG_FILES})
    ...
    execute_process(
      COMMAND ${RIFT_CMAKE_BIN_DIR}/yang_package.py ${yang_file}
    ...
    if(NOT ARG_WITHOUT_PROTO)
      ...
      if(NOT ARG_WITHOUT_GI)
        ...
        rift_symlink_python_pbcm_override(${gi_namespace} COMPONENT ${ARG_COMPONENT})
    ...
    # Make pkg-config only when installation is required
    if (ARG_COMPONENT)
      if(NOT ARG_WITHOUT_LIB)
        rift_make_pc(${name}
          LIBS -l${library_name}
          COMPONENT ${library_pkg_name})
      else()
        rift_make_pc(${name}
          COMPONENT ${library_pkg_name})
      endif()
    ...
    set(rift_meta_cmd ${RIFT_CMAKE_BIN_DIR}/rwyangmeta.py)
...
function(rift_gen_yang_tree TARGET)
  ...
  configure_file(
    ${RIFT_CMAKE_MODULE_DIR}/rift_pyang.cmake.in
...
function(rift_symlink_python_pbcm_override NAMESPACE)
  ...
  set(py3_override_dest usr/lib64/python${RIFT_PYTHON3}/site-packages/gi/overrides)
  set(py2_override_dest usr/lib64/python${RIFT_PYTHON2}/site-packages/gi/overrides)
  set(py3_dir ${CMAKE_INSTALL_PREFIX}/${py3_override_dest})
  set(py2_dir ${CMAKE_INSTALL_PREFIX}/${py2_override_dest})

  execute_process(
      COMMAND mkdir -p ${py3_dir}
      COMMAND mkdir -p ${py2_dir}
      )
  ...
  execute_process(
    COMMAND cp -f
    ${CMAKE_INSTALL_PREFIX}/usr/lib64/python${RIFT_PYTHON3}/site-packages/rift/gi/pbcm_override.py
