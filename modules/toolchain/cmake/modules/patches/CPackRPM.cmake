...
#     Mandatory : NO
#     Default   : -
#     May be used to set the compression level. If this is not specified it defaults
...
IF (CPACK_RPM_COMPRESSION_TYPE)
   ...
   IF(DEFINED CPACK_RPM_COMPRESSION_LEVEL)
     ...
     IF(CPACK_RPM_COMPRESSION_LEVEL GREATER 9)
       IF(CPACK_RPM_PACKAGE_DEBUG)
	 message("CPackRPM:Debug: Compression level should be less than 9 for lzma")
       ENDIF(CPACK_RPM_PACKAGE_DEBUG) 
     ...
     IF(CPACK_RPM_COMPRESSION_LEVEL LESS 0)
       IF(CPACK_RPM_PACKAGE_DEBUG)
	 message("CPackRPM:Debug: Compression level should be greater than 0 for lzma")
       ENDIF(CPACK_RPM_PACKAGE_DEBUG)
   ...
   IF(CPACK_RPM_PACKAGE_DEBUG)
     message("CPackRPM:Debug: Compression level set to ${CPACK_RPM_COMPRESSION_LEVEL}")
   ENDIF(CPACK_RPM_PACKAGE_DEBUG)

   IF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "lzma")
     SET(CPACK_RPM_COMPRESSION_TYPE_TMP "%define _binary_payload w${CPACK_RPM_COMPRESSION_LEVEL}.lzdio")
   ENDIF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "lzma")
   IF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "xz")
     SET(CPACK_RPM_COMPRESSION_TYPE_TMP "%define _binary_payload w${CPACK_RPM_COMPRESSION_LEVEL}.xzdio")
   ENDIF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "xz")
   IF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "bzip2")
     SET(CPACK_RPM_COMPRESSION_TYPE_TMP "%define _binary_payload w${CPACK_RPM_COMPRESSION_LEVEL}.bzdio")
   ENDIF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "bzip2")
   IF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "gzip")
     SET(CPACK_RPM_COMPRESSION_TYPE_TMP "%define _binary_payload w${CPACK_RPM_COMPRESSION_LEVEL}.gzdio")
   ENDIF(CPACK_RPM_COMPRESSION_TYPE STREQUAL "gzip")
...
#EXECUTE_PROCESS(COMMAND find . -type f -o -type l -o (-type d -a -not -name "." ${RIFT_CPACK_RPM_PATCH_FIND_FILTER})

# ajc - Having a hard time getting the above to function correctly.  Hard coding ./usr until the next victim can fix it.
EXECUTE_PROCESS(COMMAND find . -type f -o -type l -o (-type d -a -not -name "." -a -not -path "./usr")
                COMMAND sed s:.*/man.*/.*:&*:
                COMMAND sed s/\\.\\\(.*\\\)/\"\\1\"/
...
foreach(F IN LISTS CPACK_RPM_INSTALL_FILES_LIST)
  if(IS_DIRECTORY "${WDIR}/${F}")
    ...
    #if the file name is terminated with *, this could still be a directory
    string(LENGTH "${WDIR}/${F}" F_LEN)
    MATH(EXPR TEMP_LEN ${F_LEN}-1)
    string(SUBSTRING "${WDIR}/${F}" ${TEMP_LEN} -1 TEMP_SUBSTR)
    if(${TEMP_SUBSTR} STREQUAL "*")
      MATH(EXPR TEMP_LEN ${F_LEN}-1)
      string(SUBSTRING "${WDIR}/${F}" 0 ${TEMP_LEN} TEMP_FILE)
...
IF(RPMBUILD_EXECUTABLE)
  ...
  IF(CPACK_RPM_PACKAGE_DEBUG OR CPACK_RPMBUILD_EXEC_RESULT)
    ...
    MESSAGE("")
    MESSAGE("--------------------------------------------------------------")
    MESSAGE("CPackRPM:Debug: *** COMMAND STANDARD OUTPUT: ***")
    MESSAGE("--------------------------------------------------------------")
    MESSAGE("")
    MESSAGE("${RPMBUILDOUT} ")
    MESSAGE("")
    ...
    MESSAGE("")
    MESSAGE("--------------------------------------------------------------")
    MESSAGE("CPackRPM:Debug: *** COMMAND STANDARD ERROR: ***")
    MESSAGE("--------------------------------------------------------------")
    MESSAGE("")
    MESSAGE("${RPMBUILDERR} ")
    MESSAGE("")
