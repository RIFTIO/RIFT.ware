...
def get_supermodule_hash():
    """ Call the generate_supermodule_hash.sh script in the current directory.
    This script prints out a hash that represents all files in the superproject
    that could affect the output of what a submodule produces.
    """
    supermodule_hash_cmd = os.path.join(THIS_DIR, "generate_supermodule_hash.sh")
    submodule_hash = subprocess.check_output(supermodule_hash_cmd, shell=True)

    return submodule_hash
...
def submodule_hash(filename, supermodule_hash, submodule, outdir):
    ...
    Since the supermodule also depends on cmake/build files in the supermodule,
    use a supermodule hash when calculating the submodule hash.  This ensures that
    the submodule caches are flushed when a file that could affect its output is changed.
    """
    ...
    # Finally, input the supermodule hash into the hash function
    s.update(supermodule_hash)
...
def main():
    ...
    supermodule_hash = get_supermodule_hash()
    ...
    if cmdargs.print_hash:
        h=submodule_hash(cmdargs.dependency_file, supermodule_hash, cmdargs.submodule, cmdargs.output_dir)
        sys.stdout.write(h)
